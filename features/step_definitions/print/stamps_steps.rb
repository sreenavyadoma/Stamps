...
Then /^Postage Stamps: Set Serial Number to (.*)/ do |serial|
  ...
  web_apps.mail = web_apps.mail if web_apps.mail.nil?

  if serial == 'random'
    serial = "B#{Random.rand(10000..99999)}"
  end
  serial_prefix = serial.split('')[0]
  web_apps.mail.serial.set serial
  logger.info "Serial prefix: #{serial_prefix}"
  logger.info "Preview image: #{(web_apps.mail.form_view.preview_image(serial_prefix).present?)?"YES": "NO"}"
end
...
Then /^Create page objects for Print On Shipping Label$/ do
  ...
  shipping_label = web_apps.mail.print_on "Shipping Label - 8 ½” x 11” Paper"
