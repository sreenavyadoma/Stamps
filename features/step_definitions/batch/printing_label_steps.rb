Then /^View Print On option (.*) tooltip$/ do |label|
  log "Step: View Print On option #{label} tooltip"
...
Then /^Expect (.*) tool-tip to contain \"(.*)\"$/ do |label, tooltip_message|
  log "Step: Expect #{label} tool-tip to contain #{tooltip_message}"
  log label
  log tooltip_message
end
...
Then /^Select new option \"(.*)\"$/ do |label|
  log "Step: Select new option #{label}"
  @print_window = batch.toolbar.print
end
...
Then /^Expect a print of \"(.*)\"$/ do |label|
  log "Step: Expect a print of #{label}"
  @print_window = batch.toolbar.print
end


Then /^Expect a print Sample of \"(.*)\"$/ do |label|
  log "Step: Expect a print Sample of #{label}"
  @print_window = batch.toolbar.print
...
Then /^Expect system will only print first order$/ do |label|
  log "Step: Expect system will only print first order"
  @print_window = batch.toolbar.print
end
