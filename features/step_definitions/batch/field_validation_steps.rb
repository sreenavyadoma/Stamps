Given /^Expect Single Order Form (.+) text box data error tooltip to be \"(.+)\"$/ do |field, value|
  ...
  case field.downcase
    when "phone"
      ...
      browser_error_message = phone_text_box.attribute_value "dev-errorqtip"
    ...
    when "email"
      ...
      browser_error_message = email_text_box.attribute_value "dev-errorqtip"
      browser_error_message.should include value
      log "Error message is #{browser_error_message}"
    when "length"
      length_text_box = @single_order_form.length
      browser_error_message = length_text_box.attribute_value "dev-errorqtip"
      browser_error_message.should include value
      log "Error message is #{browser_error_message}"
    when "width"
      width_text_box = @single_order_form.width
      browser_error_message = width_text_box.attribute_value "dev-errorqtip"
      browser_error_message.should include value
      log "Error message is #{browser_error_message}"
    when "height"
      height_text_box = @single_order_form.height
      browser_error_message = height_text_box.attribute_value "dev-errorqtip"
      browser_error_message.should include value
      log "Error message is #{browser_error_message}"
    else
