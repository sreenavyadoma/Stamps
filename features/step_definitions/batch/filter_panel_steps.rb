...
Then /^Expect system shows name of (.*) filter in closed panel$/ do |filter|
  log.info "Expectation: Expect system shows name of #{filter} filter in closed panel"
...
Then /^Expect system changes the panel arrow so that it is pointing to the (.*) direction$/ do |direction|
  log.info "Expectation: Expect system changes the panel arrow so that it is pointing to the #{direction} direction"
...
Then /^Expect system updates the grid to show only orders that match the (.*) filter$/ do |filter|
  log.info "Expectation: Expect system updates the grid to show only orders that match the #{filter} filter"
  actual = batch.filter.is_order_grid_filtered(filter)
...
Then /^Expect system selects the (.*) filter by default$/ do |filter|
  log.info "Expectation: Expect system selects the #{filter} filter by default"
  actual = batch.filter.get_selected_filter_text
  actual.should eql filter
end
...
Then /^Expect printed Order ID does not exist in Awaiting Shipment tab$/ do
  log.info "Expectation: Expect printed Order ID does not exist in Awaiting Shipment tab"
...
Then /^Expect printed Order ID exists in Shipped tab$/ do
  log.info "Expectation: Expect printed Order ID exists in Shipped tab"
  grid = batch.shipped
...
Then /^Expect all printed Order IDs exist in Shipped tab$/ do
  log.info "Expectation: Expect all printed Order IDs exist in Shipped tab"
  grid = batch.shipped
