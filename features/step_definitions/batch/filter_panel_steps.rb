Then /^Expect system shows name of (.*) filter in closed panel$/ do |filter|
  log "Step: Expect system shows name of #{filter} filter in closed panel"
...
Then /^Expect system changes the panel arrow so that it is pointing to the (.*) direction$/ do |direction|
  log "Step: Expect system changes the panel arrow so that it is pointing to the #{direction} direction"
...
Then /^Expect system updates the grid to show only orders that match the (.*) filter$/ do |filter|
  log "Step: Expect system updates the grid to show only orders that match the #{filter} filter"
...
Then /^Expect system (.*) "Print" button in control ribbon$/ do |status|
  log "Step: Expect system #{status} Print button in control ribbon"
...
Then /^Expect system displays expanded filters panel$/ do
  log "Step: Expect system displays expanded filters panel"
