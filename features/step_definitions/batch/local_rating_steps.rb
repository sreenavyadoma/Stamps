Then /^Verify Local Rating$/ do |table|
  ...
  CSV.open("local_rating_results.csv", "wb") do |csv|
    log "| ship_from | ship_to | service | weight_lbs | weight_oz | length | height | width | tracking | total |"
    ...
    parameter_array.each_with_index { |element, index|
      log "##{index} | #{element["ship_from"]} | #{element["ship_to"]} | #{element["service"]} | #{element["weight_lbs"]} | #{element["weight_oz"]} | #{element["length"]} | #{element["height"]} | #{element["width"]} | #{element["tracking"]} | #{element["total"]} |"

      csv << [index+1, element["ship_from"], element["ship_to"], element["service"], element["weight_lbs"], element["weight_oz"], element["length"], element["height"], element["width"], element["tracking"], element["total"]]
    }
  end
  ...
  10.times { |counter|
    ...
    log_expectation_eql "#{counter}. Total Cost", element["total"], total
    break if total.eql? element["total"]
  }
