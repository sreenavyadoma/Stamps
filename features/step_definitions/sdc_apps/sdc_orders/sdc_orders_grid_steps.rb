...
Then /^check grid order id(?:| (\d*))$/ do |order_id|
  ...
  column = OrdersGrid.grid_column(:checkbox)
...
When /^check row (\d+)$/ do |row|
  OrdersGrid.body.wait_until_present(timeout: 10)
  checkbox = OrdersGrid.grid_column(:checkbox).checkbox_row(row)
  checkbox.check
...
Then /^expect orders grid store is (.*)$/ do |str|
  ...
  result = OrdersGrid.grid_column(:store).data(order_id)
...
Then /^expect orders grid order id is (?:correct|(.*))$/ do |order_id|
  ...
  result = OrdersGrid.grid_column(:order_id).data(order_id)
...
Then /^expect orders grid ship cost is (?:correct|(.*))$/ do |str|
  ...
  result = OrdersGrid.grid_column(:ship_cost).data(order_id).parse_digits
...
Then /^expect orders grid ship date is (?:correct|(.*))$/ do |str|
  ...
  column = OrdersGrid.grid_column(:ship_date)
...
Then /^expect orders grid date printed is (?:correct|(.*))$/ do |str|
  ...
  column = OrdersGrid.grid_column(:date_printed)
  expect(column).to be_present
...
Then /^expect orders grid order date is populated$/ do
  ...
  result = OrdersGrid.grid_column(:order_date).data(order_id)
...
Then /^expect orders grid age is (.+)$/ do |str|
  ...
  result = OrdersGrid.grid_column(:age).data(order_id)
  expect(result).to eql str
end
...
Then /^expect orders grid recipient is (?:correct|(.*))$/ do |str|
  ...
  result = OrdersGrid.grid_column(:recipient).data(order_id)
  expect(result.strip).to eql str
end
...
Then /^expect orders grid company is (?:correct|(.*))$/ do |str|
  ...
  order_id = TestData.hash[:order_id].values.last
  result = OrdersGrid.grid_column(:company).data(order_id)
  expect(result.strip).to eql str
end
...
Then /^expect orders grid address is (?:correct|(.*))$/ do |str|
  ...
  order_id = TestData.hash[:order_id].values.last
  result = OrdersGrid.grid_column(:address).data(order_id)
  expect(result.strip).to eql str
end
...
Then /^expect orders grid city is (?:correct|(.*))$/ do |str|
  ...
  order_id = TestData.hash[:order_id].values.last
  result = OrdersGrid.grid_column(:city).data(order_id)
  expect(result.strip).to eql str
end
...
Then /^expect orders grid state is (?:correct|(.*))$/ do |str|
  ...
  order_id = TestData.hash[:order_id].values.last
  result = OrdersGrid.grid_column(:state).data(order_id)
  expect(result.strip).to eql str
end
...
Then /^expect orders grid zip is (?:correct|(.*))$/ do |str|
  ...
  order_id = TestData.hash[:order_id].values.last
  result = OrdersGrid.grid_column(:zip).data(order_id)
...
Then /^expect orders grid country is (?:correct|(.*))$/ do |str|
  ...
  order_id = TestData.hash[:order_id].values.last
  result = OrdersGrid.grid_column(:country).data(order_id)
  expect(result.strip).to include(str)
end
...
Then /^expect orders grid email is (?:correct|(.*))$/ do |str|
  ...
  order_id = TestData.hash[:order_id].values.last
  result = OrdersGrid.grid_column(:email).data(order_id)
  expect(result.strip).to eql str
end
...
Then /^expect orders grid phone is (?:correct|(.*))$/ do |str|
  ...
  order_id = TestData.hash[:order_id].values.last
  result = OrdersGrid.grid_column(:phone).data(order_id)
  expect(result.strip).to eql str
end
...
Then /^expect orders grid weight is (\d+) lb. (\d+) oz.$/ do |lb, oz|
  str = "#{lb} lbs. #{oz} oz."
  order_id = TestData.hash[:order_id].values.last
  result = OrdersGrid.grid_column(:weight).data(order_id)
  expect(result).to eql str
end
...
Then /^expect orders grid pounds is (?:correct|(.*))$/ do |str|
  ...
  order_id = TestData.hash[:order_id].values.last
  result = OrdersGrid.grid_column(:weight).lb(order_id)
...
Then /^expect orders grid ounces is (?:correct|(.*))$/ do |str|
  ...
  order_id = TestData.hash[:order_id].values.last
  result = OrdersGrid.grid_column(:weight).oz(order_id)
  expect(result).to eql str.to_i
end
...
Then /^expect orders grid qty is (?:correct|(.*))$/ do |str|
  ...
  order_id = TestData.hash[:order_id].values.last
  result = OrdersGrid.grid_column(:qty).data(order_id)
  expect(result.to_i).to eql str.to_i
end

Then /^expect orders grid item sku is (.+)$/ do |str|
  order_id = TestData.hash[:order_id].values.last
  result = OrdersGrid.grid_column(:item_sku).data(order_id)
  expect(result.strip).to eql str
end

Then /^expect orders grid item name is (.+)$/ do |str|
  order_id = TestData.hash[:order_id].values.last
  column = OrdersGrid.grid_column(:item_name)
...
Then /^expect orders grid service is (?:correct|(.*))$/ do |str|
  ...
  order_id = TestData.hash[:order_id].values.last
  result = OrdersGrid.grid_column(:service).data(order_id)
  expect(result).to eql(str)
end
...
Then /^expect orders grid service ship-from is (?:correct|(.*))$/ do |str|
  ...
  result = OrdersGrid.grid_column(:ship_from).data(order_id)
  expect(result.strip).to eql str
end
...
Then /^expect orders grid insured value is (?:correct|(.*))$/ do |str|
  ...
  result = OrdersGrid.grid_column(:insured_value).data(order_id)
...
Then /^expect orders grid reference number is (?:correct|(.*))$/ do |str|
  ...
  result = OrdersGrid.grid_column(:reference_no).data(order_id)
  expect(result.strip).to eql str
end
...
Then /^expect orders grid tracking service is (?:correct|(.*))$/ do |str|
  ...
  result = OrdersGrid.grid_column(:tracking_service).data(order_id)
  expect(result.strip).to eql str
end
...
Then /^expect orders grid order status is (.+)$/ do |str|
  ...
  result = OrdersGrid.grid_column(:order_status).data(order_id)
  expect(result.strip).to eql str
end
...
Then /^expect orders grid tracking number is populated$/ do
  ...
  result = OrdersGrid.grid_column(:tracking_no).data(order_id)
  expect(result.strip).not_to eql ''
end
...
Then /^[Ee]xpect [Oo]rders [Gg]rid Ship Cost error to contain \"(.*)\"$/ do |str|
  grid_order_id = OrdersGrid.grid_column(:order_id).text_at_row 1
  ship_cost_error = OrdersGrid.grid_column(:ship_cost).data_error grid_order_id
  expect(ship_cost_error).to include(str)
  ship_cost_error = OrdersGrid.grid_column(:ship_cost).data_error "81453"
  ship_cost_error = OrdersGrid.grid_column(:ship_cost).data_error "81408"
  ship_cost_error = OrdersGrid.grid_column(:ship_cost).data_error "81407"
end
...
Then /^[Ee]xpect [Oo]rders [Gg]rid ship cost data error tooltip is \"(.*)\"$/ do |str|
  ...
  grid_order_id = OrdersGrid.grid_column(:order_id).text_at_row 1
  grid_ship_cost = OrdersGrid.grid_column(:ship_cost).data grid_order_id
