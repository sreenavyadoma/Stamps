...
Then /^Import Orders: Import$/ do
  ...
  SdcLogger.debug "Success modal is present after #{import_time} seconds"
  ...
  SdcLogger.debug "Import Orders File: #{import_time_loc}"
  ...
  SdcLogger.debug "Import Time Saved to CSV file"
  csv_file.close
end
...
Then /^Import Orders: Randomize data in (.*)$/ do |filename|
  ...
  #SdcLog.step "File location is #{'import_orders_dir'+ENV['URL']}"
  ...
  CSV.open(import_new_loc, "w+") do |csv_out|
    ...
    SdcLogger.debug "Orders info in #{filename} has been randomized"
  end
end
...
Then /^Import Orders: File Upload: Set Filename to (.*)$/ do |filename|
  ...
  SdcLogger.debug "Import File:  #{import_orders_loc}"
