...
Then /^[Aa]dd random Ship-from address from (.*)$/ do |address|
  TestData.store[:ship_from_address] = TestHelper.address_helper_zone(address, modal_param.test_env)
...
Then /^[Oo]n Manage Shipping Address modal, add address (\w+)$/ do |address|
  TestData.store[:ship_from_address] = address.include?('random ship from zone 1 through 4') ? TestHelper.rand_ship_from_zone_1_4(modal_param.test_env) : address
