...
Then /^[Aa]dd random Ship-from address from (.*)$/ do |address|
  test_param[:ship_from_address]=TestHelper.address_helper_zone(address)
...
Then /^[Oo]n Manage Shipping Address modal, add address (\w+)$/ do |address|
  test_param[:ship_from_address]=(address.include? 'random ship from zone 1 through 4')?(TestHelper.rand_ship_from_zone_1_4):address
