...
Then /^[Oo]n Manage Shipping Address modal, delete all addresses$/ do
  ...
  stamps.orders.single_order_details.single_ship_from.select("Manage Shipping Addresses").delete_all.close_window
end

Then /^[Oo]n Manage Shipping Address modal, delete Row (\d+) from Manage Shipping Addresses Modal/ do |row|
  stamps.orders.single_order_details.single_ship_from.select("Manage Shipping Addresses").delete_row(row)
end

Then /^[Oo]n Manage Shipping Address modal, edit address for name = \"(.*)\", company = \"(.*)\" and city = \"(.*)\" to;$/ do |name, company, city, new_address|
  ship_address_modal = stamps.orders.single_order_details.single_ship_from.select "Manage Shipping Addresses"
...
Then /^[Aa]dd random Ship-from address from (.*)$/ do |address|
  ...
  stamps.orders.single_order_details.single_ship_from.select("Manage Shipping Addresses").add.ship_from_address(test_param[:ship_from_address])
end
...
Then /^[Oo]n Manage Shipping Address modal, add address (\w+)$/ do |address|
  ...
  stamps.orders.single_order_details.single_ship_from.select("Manage Shipping Addresses").add(test_param[:ship_from_address])
end

Then /^[Oo]n Manage Shipping Address modal, set Manage Shipping Addresses to$/ do
  stamps.orders.single_order_details.single_ship_from.select("Manage Shipping Addresses").add table.hashes.first
end
