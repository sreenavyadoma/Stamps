...
Then /^Ship-From: Delete all addresses$/ do
  ...
  stamps.orders.details.ship_from.select("Manage Shipping Addresses...").delete_all.close_window
end
...
Then /^Ship-From: Delete Row (\d+) from Manage Shipping Addresses Modal/ do |row|
  ...
  stamps.orders.details.ship_from.select("Manage Shipping Addresses...").delete_row(row)
end
...
Then /^Ship-From: Edit address for name = \"(.*)\", company = \"(.*)\" and city = \"(.*)\" to;$/ do |name, company, city, new_address|
  ...
  ship_address_modal = stamps.orders.details.ship_from.select "Manage Shipping Addresses..."
...
Then /^Ship-From: Add address$/ do |ship_from|
  ...
  add_form = stamps.orders.details.ship_from.select("Manage Shipping Addresses...")
...
Then /^Ship-From: Add address (\w+)$/ do |address|
  ...
  @ship_from_address = stamps.orders.details.ship_from.select("Manage Shipping Addresses...").add(ship_from)
...
Then /^Ship-From: Set Manage Shipping Addresses to$/ do
  ...
  stamps.orders.details.ship_from.select("Manage Shipping Addresses...").add table.hashes.first
end
...
Then /^Ship-From: Expect selection has (.*)$/ do |address|
  ...
  stamps.orders.details.ship_from.select("Manage Shipping Addresses...").address_located?(@ship_from_address).should be true
end