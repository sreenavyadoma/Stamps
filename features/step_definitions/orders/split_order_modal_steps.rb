...
Then /^[Ee]xpect [Ss]plit [Oo]rder [Mm]odal original order id is correct$/ do
  expect(stamps.orders.modals.split_order_modal.original_order_id).to eql TestData.store[:order_id].values.last #confirm original order id in Split order modal matches id of the last created order
end

Then /^[Ee]xpect [Ss]plit [Oo]rder [Mm]odal default new order id is correct$/ do
  expect(stamps.orders.modals.split_order_modal.new_order_id.text).to eql (TestData.store[:order_id].values.last + "-1") #confirm that the default new order id is the same as the original id with "-1" appended to it
end
...
Then /^[Cc]lick [Ss]plit [Oo]rder modal confirm button$/ do
  TestData.store[:split_order_id] = stamps.orders.modals.split_order_modal.new_order_id.text  #save the new order id
...
Then /[Ee]xpect new order from [Ss]plit [Oo]rder modal is selected in orders grid$/ do
  expect(stamps.orders.order_details.toolbar.order_id.text).to eql TestData.store[:split_order_id] #confirm the order id in the single order details form matches the saved new order id
end