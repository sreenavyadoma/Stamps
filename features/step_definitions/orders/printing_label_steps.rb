Then /^View Print On option (.*) tooltip$/ do |label|
  #StampsTest.log.step "View Print On option #{label} tooltip"
...
Then /^[Ee]xpect (.*) tool-tip to contain \"(.*)\"$/ do |label, tooltip_message|
  #StampsTest.log.step "Expect #{label} tool-tip to contain #{tooltip_message}"
  StampsTest.log.step label
  StampsTest.log.step tooltip_message
end
