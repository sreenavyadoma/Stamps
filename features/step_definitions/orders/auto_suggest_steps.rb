Then /^Details: Set Ship-To auto-suggest address to partial name (.*)$/ do |partial_name|
  ...
  @auto_suggest = stamps.orders.details.ship_to.address.auto_suggest.set @auto_suggest_partial_name
end
...
Then /^Details: Set International Ship-To auto-suggest address to partial name (.*)$/ do |partial_name|
  ...
  @auto_suggest = stamps.orders.details.ship_to.international.auto_suggest.set @auto_suggest_partial_name
end
...
Then /^Expect Auto Suggest name shows (.*) for entry (.*)$/ do |value, entry|
  ...
  actual =  stamps.orders.details.get_auto_suggest_name entry
...
Then /^Expect Auto Suggest location shows (.*) for entry (.*)$/ do |value, entry|
  ...
  actual =  stamps.orders.details.get_auto_suggest_location entry
  actual.should eql value
end
...
Then /^Select entry (.*) in the auto suggest drop down list$/ do |entry|
  ...
  stamps.orders.details.click_auto_suggest_name entry
end
...
Then /^Expect Domestic Address is (.*)$/ do |value|
  ...
  begin
    ...
    actual = stamps.orders.details.ship_to.address.text_area.text
...
Then /^Expect Domestic Phone is (.*)$/ do |value|
  ...
  actual =  stamps.orders.details.ship_to.address.phone.text
  actual.should eql value
end

Then /^Expect Domestic Email is (.*)$/ do |value|
  logger.step "Expect Domestic Email is #{value}"
  actual =  stamps.orders.details.ship_to.address.email.text
  actual.should eql value
end
