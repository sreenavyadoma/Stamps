...
Then /^Print: Expect Hide Mail Value Checkbox is checked$/ do
  ...
  30.times do
    web_apps.orders.toolbar.print_btn.print_modal.click
    break if web_apps.orders.toolbar.print_btn.print_modal.print_options.hide_postage_value.checked?
  end
  web_apps.orders.toolbar.print_btn.print_modal.print_options.hide_postage_value.checked?.should be true
end
...
When /^Print: Select left-side label$/ do
  ...
  web_apps.orders.toolbar.print_btn.print_modal.starting_label.left
end

When /^Print: Select right-side label$/ do
  logger.step "Print: Select - Right side label"
  web_apps.orders.toolbar.print_btn.print_modal.starting_label.right
end

Then /^Print: Expect right-side label selected$/ do
  logger.step "Print: Expect right-side label selected"
  web_apps.orders.toolbar.print_btn.print_modal.starting_label.right_selected?.should be true
end

Then /^Print: Expect left-side label selected$/ do
  logger.step "Print: Expect left-side label selected"
  web_apps.orders.toolbar.print_btn.print_modal.starting_label.left_selected?.should be true
end

Then /^Print: Expect Ship Date is (\d+) day\(s\) from today/ do |day|
  logger.step "Print: Expect Ship Date is #{day} day(s) from today"
  web_apps.orders.toolbar.print_btn.print_modal.ship_date.text.should eql ParameterHelper.date_printed(day)
end
...
Then /^Print: Expect Printing On Label is (.*)$/ do |expectation|
  logger.step "Print: Set Printing On #{expectation}"
  web_apps.orders.toolbar.print_btn.print_modal.printing_on.label.text.should eql expectation
end
...
Then /^Print expecting error (.*)$/ do |error_message|
  ...
  modal = web_apps.orders.toolbar.print_btn.print_modal.print_expecting_error
...
Then /^Print expecting invalid address error$/ do
  ...
  web_apps.orders.toolbar.print_btn.print_modal.print_invalid_address.close
end

When /^Print expecting rating error$/ do
  logger.step "Print expecting rating error"
  modal = web_apps.orders.toolbar.print_btn.print_modal.print_expecting_rating_error
  actual = modal.error_message
  modal.close
...
Then /^Print: Expect number of required label sheets is (\d+)$/ do |sheets|
  ...
  web_apps.orders.toolbar.print_btn.print_modal.labels_required.should eql sheets
end

Then /^Print raises a Printing Error/ do
  logger.step "Print raises a Printing Error"
  web_apps.orders.print.print_sample_expecting_error.should raise_error(PrintingError)
end
...
Then /^Print: Print Sample raises a Printing Error/ do
  ...
  web_apps.orders.toolbar.print_btn.print_modal.print_sample_expecting_error.should raise_error(PrintingError)
end
