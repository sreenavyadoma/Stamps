...
Then /^[Ee]xpect Ship Cost equals Total amount$/ do
  10.times {
    begin
      break if stamps.orders.orders_grid.grid_column(:ship_cost).data(test_param[:order_id][1]).eql?(stamps.orders.single_order_details.footer.total_ship_cost)
    rescue
      #ignore
    end
  }
  expect(stamps.orders.orders_grid.grid_column(:ship_cost).data(test_param[:order_id][1])).to eql(stamps.orders.single_order_details.footer.total_ship_cost)
end
...
Then /^[Ee]xpect Print modal Total Cost is \$([0-9.]*)$/ do |expectation|
  begin
    print_window = stamps.orders.orders_toolbar.toolbar_print.orders_print_modal
