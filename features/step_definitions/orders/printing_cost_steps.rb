...
Then /^[Ee]xpect Ship Cost equals Total amount$/ do
  10.times {
    begin
      break if stamps.orders.orders_grid.grid_column(:ship_cost).data(test_param[:order_id].values.last).eql?(stamps.orders.order_details.footer.total_ship_cost)
    rescue
      #ignore
    end
  }
  expect(stamps.orders.orders_grid.grid_column(:ship_cost).data(test_param[:order_id].values.last)).to eql(stamps.orders.order_details.footer.total_ship_cost)
end
