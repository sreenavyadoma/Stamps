    ...
    begin
      if row_number > 0
        ...
        if row[zone_column] == nil
          ...
          SdcLogger.info "#{"#" * 10} Price: #{TestData.store[:result_sheet][row_number, TestData.store[:result_sheet_columns][:zone]]}"
          SdcLogger.info "#{"#" * 10} "
          SdcLogger.info "#{"#" * 50}"
          ...
          SdcLogger.info "#{"#" * 10} "
          SdcLogger.info "#{"#" * 10} Weight: #{TestData.store[:result_sheet][row_number, TestData.store[:result_sheet_columns][:weight]]}"
          SdcLogger.info "#{"#" * 10} Selected Service: #{TestData.store[:result_sheet][row_number, TestData.store[:result_sheet_columns][:service_selected]]}"
          SdcLogger.info "#{"#" * 10} Ship-To Address: #{TestData.store[:address]}" if SdcEnv.sdc_app == :mail
          SdcLogger.info "#{"#" * 10} Ship-To Address: #{TestData.store[:full_name]}, #{TestData.store[:street_address]}, #{TestData.store[:city]}, #{TestData.store[:state]}, #{TestData.store[:zip]}" if SdcEnv.sdc_app == :orders
          SdcLogger.info "#{"#" * 10} #{"*" * 5} Test #{TestData.store[:result_sheet][row_number, TestData.store[:result_sheet_columns][:status]] } - Expected #{TestData.store[:result_sheet][row_number, TestData.store[:result_sheet_columns][:zone]]}, Got #{TestData.store[:result_sheet][row_number, TestData.store[:result_sheet_columns][:total_ship_cost]]} #{"*" * 5}"
          SdcLogger.info "#{"#" * 10} "
        end
