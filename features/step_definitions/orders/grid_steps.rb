...
Then /^Grid: Check Order ID (.*)$/ do |order_id|
  ...
  @orders_test_data[:order_id] = order_id
  # Check if Order ID exists, fail test if it does not
  exists = web_apps.orders.grid.order_id.exist? @orders_test_data[:order_id]
  expectation = "exists"
  expectation = "does not exists" unless exists
  expectation.should eql "exists"
  logger.step "Order ID #{@orders_test_data[:order_id]} #{expectation}!"
  web_apps.orders.grid.checkbox.check_order @orders_test_data[:order_id]
end
...
Then /^Grid: Expect Store is (.*)$/ do |expectation|
  ...
  actual_value = web_apps.orders.grid.store.data @orders_test_data[:order_id]
...
Then /^Grid: Expect Ship Cost is the same as Details Form Ship Cost$/ do
  ...
  grid_ship_cost = web_apps.orders.grid.ship_cost.data @orders_test_data[:order_id]
...
Then /^Grid: Uncheck Saved Order ID$/ do
  logger.step "Grid: Uncheck Saved Order ID#{@orders_test_data[:order_id]}"
  web_apps.orders.grid.checkbox.uncheck_order @orders_test_data[:order_id]
  web_apps.orders.grid.checkbox.order_checked?(@orders_test_data[:order_id]).should be false
end

Then /^Grid: Check Saved Order ID$/ do
  logger.step "Grid: Check Saved Order ID #{@orders_test_data[:order_id]}"
  web_apps.orders.grid.checkbox.check_order @orders_test_data[:order_id]
  web_apps.orders.grid.checkbox.order_checked?(@orders_test_data[:order_id]).should be true
end

Then /^Grid: Check Environment Order ID$/ do
  @orders_test_data[:order_id] = (ENV['ORDER_ID'].nil?)?"":ENV['ORDER_ID']
  expectation = "ENV['ORDER_ID'] is not nil"
  expectation = "ENV['ORDER_ID'] is nil! This test needs a value for this variable." if @orders_test_data[:order_id].size == 0
  expectation.should eql "ENV['ORDER_ID'] is not nil"

  logger.step "Grid: Check Order ID #{@orders_test_data[:order_id]}"
  step "Grid: Check Order ID #{@orders_test_data[:order_id]}"
end
...
Then /^Grid: Expect Date Printed for this order is today$/ do
  ...
  grid_print_date = grid.date_printed.data @orders_test_data[:order_id] # Dec 3
  expectation_print_date = Date.today.strftime "%b %-d"

  logger.step "Order ID:  #{@orders_test_data[:order_id]} - Orders Grid Date Printed:  #{grid_print_date} - Today's date:  #{expectation_print_date}"
...
Then /^Grid: Expect Ship Date for this order is today plus (\d+)$/ do |day|
  ...
  10.times{
    grid.order_id.sort_descending
    grid_ship_date = grid.ship_date.data @orders_test_data[:order_id] # Dec 3
    logger.step "Order ID:  #{@orders_test_data[:order_id]} - Print Modal Saved Ship Date: #{@ship_date} - Orders Grid Ship Date:  #{grid_ship_date}"

    break if grid_ship_date == expectation_ship_date
  }

  grid_ship_date = grid.ship_date.data @orders_test_data[:order_id]
...
Then /^Grid: Expect Age is (.+)$/ do |expectation|
  ...
  @orders_test_data[:order_id].should be_truthy
  10.times do
    break if web_apps.orders.grid.age.data(@orders_test_data[:order_id]).eql? expectation
  end
  actual = web_apps.orders.grid.age.data(@orders_test_data[:order_id])
...
Then /^Grid: Expect Order Date is populated$/ do
  ...
  @orders_test_data[:order_id].should be_truthy
  5.times do
    break if web_apps.orders.grid.order_date.data(@orders_test_data[:order_id]).size > 4
  end
  actual = web_apps.orders.grid.order_date.data(@orders_test_data[:order_id])
...
Then /^Grid: Expect Recipient is (.+)$/ do |expectation|
  ...
  @orders_test_data[:order_id].should be_truthy
  10.times do
    break if web_apps.orders.grid.recipient.data(@orders_test_data[:order_id]).eql? expectation
  end
  actual = web_apps.orders.grid.recipient.data @orders_test_data[:order_id]
  logger.step "Test #{(actual==expectation)?"Passed":"Failed"}"
  actual.should eql expectation
end
...
Then /^Grid: Expect Company is (.+)$/ do |expectation|
  ...
  10.times do
    break if web_apps.orders.grid.company.data(@orders_test_data[:order_id]).eql? expectation
  end
  actual = web_apps.orders.grid.company.data(@orders_test_data[:order_id])
  logger.step "Test #{(actual==expectation)?"Passed":"Failed"}"
  actual.should eql expectation
end
...
Then /^Grid: Expect Address is (.+)$/ do |expectation|
  ...
  10.times do
    break if web_apps.orders.grid.address.data(@orders_test_data[:order_id]).eql? expectation
  end
  actual = web_apps.orders.grid.address.data(@orders_test_data[:order_id])
  logger.step "Test #{(actual==expectation)?"Passed":"Failed"}"
  actual.should eql expectation
end
...
Then /^Grid: Expect City is (.+)$/ do |expectation|
  ...
  10.times do
    break if web_apps.orders.grid.city.data(@orders_test_data[:order_id]).eql? expectation
  end
  actual = web_apps.orders.grid.city.data(@orders_test_data[:order_id])
  logger.step "Test #{(actual==expectation)?"Passed":"Failed"}"
  actual.should eql expectation
end
...
Then /^Grid: Expect State is (.+)$/ do |expectation|
  ...
  10.times do
    break if web_apps.orders.grid.state.data(@orders_test_data[:order_id]).eql? expectation
  end
  actual = web_apps.orders.grid.state.data(@orders_test_data[:order_id])
  logger.step "Test #{(actual==expectation)?"Passed":"Failed"}"
  actual.should eql expectation
end
...
Then /^Grid: Expect Zip is (.+)$/ do |expectation|
  ...
  10.times do
    break if web_apps.orders.grid.zip.data(@orders_test_data[:order_id]).eql? expectation
  end
  actual = web_apps.orders.grid.zip.data(@orders_test_data[:order_id])
  logger.step "Test #{(actual==expectation)?"Passed":"Failed"}"
...
Then /^Grid: Expect Country is (.+)$/ do |expectation|
  ...
  10.times do
    break if web_apps.orders.grid.country.data(@orders_test_data[:order_id]).eql? expectation
  end
  actual = web_apps.orders.grid.country.data(@orders_test_data[:order_id])
  logger.step "Test #{(actual==expectation)?"Passed":"Failed"}"
  actual.should include expectation
  actual.split(" ").last.should eql expectation
end
...
Then /^Grid: Expect Email is (.+)$/ do |expectation|
  ...
  10.times do
    break if web_apps.orders.grid.email.data(@orders_test_data[:order_id]).eql? expectation
  end
  actual = web_apps.orders.grid.email.data @orders_test_data[:order_id]
  logger.step "Test #{(actual==expectation)?"Passed":"Failed"}"
  actual.should eql expectation
end
...
Then /^Grid: Expect Phone is (.+)$/ do |expectation|
  ...
  10.times do
    break if web_apps.orders.grid.phone.data(@orders_test_data[:order_id]).eql? expectation
  end
  actual = web_apps.orders.grid.phone.data @orders_test_data[:order_id]
  logger.step "Test #{(actual==expectation)?"Passed":"Failed"}"
  actual.should eql expectation
end
...
Then /^Grid: Expect Pounds is (\d+)$/ do |expectation|
  ...
  10.times do
    break if web_apps.orders.grid.weight.lbs(@orders_test_data[:order_id]).eql? expectation
  end
  actual = web_apps.orders.grid.weight.lbs @orders_test_data[:order_id]
  logger.step "Test #{(actual==expectation)?"Passed":"Failed"}"
  actual.should eql expectation
end
...
Then /^Grid: Expect Ounces is (\d+)$/ do |expectation|
  ...
  10.times do
    break if web_apps.orders.grid.weight.oz(@orders_test_data[:order_id]).eql? expectation
  end
  actual = web_apps.orders.grid.weight.oz(@orders_test_data[:order_id])
  logger.step "Test #{(actual==expectation)?"Passed":"Failed"}"
  actual.should eql expectation
end
...
Then /^Grid: Expect Weight is (\d+) lbs. (\d+) oz.$/ do |pounds, ounces|
  ...
  10.times do
    break if web_apps.orders.grid.weight.data(@orders_test_data[:order_id]).eql? expectation
  end
  actual = web_apps.orders.grid.weight.data @orders_test_data[:order_id]
  logger.step "Test #{(actual==expectation)?"Passed":"Failed"}"
  actual.should eql expectation
end
...
Then /^Grid: Expect Qty. is (.+)$/ do |expectation|
  ...
  10.times do
    break if web_apps.orders.grid.qty.data(@orders_test_data[:order_id]).eql? expectation
  end
  actual = web_apps.orders.grid.qty.data(@orders_test_data[:order_id])
  logger.step "Test #{(actual==expectation)?"Passed":"Failed"}"
  actual.should eql expectation
end
...
Then /^Grid: Expect Item SKU is (.+)$/ do |expectation|
  ...
  10.times do
    break if web_apps.orders.grid.item_sku.data(@orders_test_data[:order_id]).eql? expectation
  end
  actual = web_apps.orders.grid.item_sku.data(@orders_test_data[:order_id])
  logger.step "Test #{(actual==expectation)?"Passed":"Failed"}"
  actual.should eql expectation
end
...
Then /^Grid: Expect Item Name is (.+)$/ do |expectation|
  ...
  10.times do
    break if web_apps.orders.grid.item_name.data(@orders_test_data[:order_id]).eql? expectation
  end
  actual = web_apps.orders.grid.item_name.data(@orders_test_data[:order_id])
  logger.step "Test #{(actual==expectation)?"Passed":"Failed"}"
  actual.should eql expectation
end
...
Then /^Grid: Expect Ship From is (.+)$/ do |expectation|
  ...
  10.times do
    break if web_apps.orders.grid.ship_from.data(@orders_test_data[:order_id]).eql? expectation
  end
  actual = web_apps.orders.grid.ship_from.data(@orders_test_data[:order_id])
  logger.step "Test #{(actual==expectation)?"Passed":"Failed"}"
  actual.should eql expectation
end
...
Then /^Grid: Expect Service is (.+)$/ do |expectation|
  ...
  10.times do
    break if web_apps.orders.grid.service.data(@orders_test_data[:order_id]).eql? expectation
  end
  actual = web_apps.orders.grid.service.data(@orders_test_data[:order_id])
  logger.step "Test #{(actual==expectation)?"Passed":"Failed"}"
  actual.should eql expectation
end
...
Then /^Grid: Expect Insured Value is \$(.+)$/ do |expectation|
  ...
  10.times do
    break if web_apps.orders.grid.insured_value.data(@orders_test_data[:order_id]).eql? expectation
  end
  actual = web_apps.orders.grid.insured_value.data(@orders_test_data[:order_id])
  logger.step "Test #{(actual==expectation)?"Passed":"Failed"}"
  actual.should eql expectation
end
...
Then /^Grid: Expect Reference No. is (.+)$/ do |expectation|
  ...
  10.times do
    break if web_apps.orders.grid.reference_no.data(@orders_test_data[:order_id]).eql? expectation
  end
  actual = web_apps.orders.grid.reference_no.data(@orders_test_data[:order_id])
  logger.step "Test #{(actual==expectation)?"Passed":"Failed"}"
  actual.should eql expectation
end
...
Then /^Grid: Expect Cost Code is (.+)$/ do |expectation|
  ...
  10.times do
    break if web_apps.orders.grid.cost_code.data(@orders_test_data[:order_id]).eql? expectation
  end
  actual = web_apps.orders.grid.cost_code.data(@orders_test_data[:order_id])
  logger.step "Test #{(actual==expectation)?"Passed":"Failed"}"
  actual.should eql expectation
end
...
Then /^Grid: Expect Tracking Service is \"(.+)\"$/ do |expectation|
  ...
  10.times do
    break if web_apps.orders.grid.tracking_service.data(@orders_test_data[:order_id]) == expectation
  end
  actual = web_apps.orders.grid.tracking_service.data(@orders_test_data[:order_id])
  logger.step "Test #{(actual==expectation)?"Passed":"Failed"}"
  actual.should eql expectation
end
...
Then /^Grid: Expect Order Status is (.+)$/ do |expectation|
  ...
  10.times do
    break if web_apps.orders.grid.order_status.data(@orders_test_data[:order_id]) == expectation
  end
  actual = web_apps.orders.grid.order_status.data(@orders_test_data[:order_id])
  logger.step "Test #{(actual==expectation)?"Passed":"Failed"}"
  actual.should eql expectation
end
...
Then /^Grid: Expect Tracking Number is populated$/ do
  ...
  20.times do
    break if web_apps.orders.grid.tracking_no.data(@orders_test_data[:order_id]).length > 3
  end
  actual = web_apps.orders.grid.tracking_no.data(@orders_test_data[:order_id])
...
Then /^Grid: Expect Order Total is (.+)$/ do |expectation|
  ...
  begin
    10.times do
      break if web_apps.orders.grid.order_total.data(@orders_test_data[:order_id]).eql? expectation
     end
    actual = web_apps.orders.grid.order_total.data(@orders_test_data[:order_id])
    logger.step "Test #{(actual==expectation)?"Passed":"Failed"}"
    actual.should eql expectation
...
Then /^Expect new Order ID created$/ do
  ...
  logger.step "Saved Order ID created:  #{@orders_test_data[:order_id].to_i>0}"
  @orders_test_data[:order_id].to_i.should be > 0
end
