...
Then /^[Cc]heck [Oo]rders [Gg]rid [Cc]ached [Oo]rder [Ii][Dd](?:| (\d+))$/ do |order_id|
  ...
  expect(stamps.orders.orders_grid.column.checkbox.order_id_checked?(order_id)).to be(true), "Couldn't check Order ID #{order_id}"
end
...
Then /^[Ee]xpect [Oo]rders [Gg]rid Store is (.*)$/ do |expectation|
  ...
  expect(stamps.orders.orders_grid.column.store.data(test_param[:order_id][1])).to eql test_param[:store_name]
end
...
Then /^[Ee]xpect cached Order ID is in [Oo]rders [Gg]rid [Rr]ow (\d+)$/ do |row|
  15.times { sleep(0.25); break if stamps.orders.orders_grid.column.order_id.row(row) == test_param[:order_id][1] }
  expect(stamps.orders.orders_grid.column.order_id.row(row)).to eql test_param[:order_id][1]
end

Then /^[Ee]xpect [Oo]rders [Gg]rid Ship Cost is the same as Details Form Ship Cost$/ do
  expect(stamps.orders.single_order_details.footer.total_ship_cost).to eql(stamps.orders.orders_grid.column.ship_cost.data(test_param[:order_id][1]))
end
...
Then /^[Ee]xpect [Oo]rders [Gg]rid Date Printed for this order is today$/ do
  expect(stamps.orders.orders_grid.column.date_printed.data(test_param[:order_id][1])).to eql(Date.today.strftime "%b %-d")
end
...
Then /^[Ee]xpect [Oo]rders [Gg]rid Ship Date for this order is today plus (\d+)$/ do |day|
  ...
  expect(stamps.orders.orders_grid.column.ship_date.data(test_param[:order_id][1])).to eql expectation
end
...
Then /^[Ee]xpect [Oo]rders [Gg]rid Age is (.+)$/ do |expectation|
  ...
  10.times { break if stamps.orders.orders_grid.column.age.data(test_param[:order_id][1]).eql? expectation }
  expect(stamps.orders.orders_grid.column.age.data(test_param[:order_id][1])).to eql expectation
end

Then /^[Ee]xpect [Oo]rders [Gg]rid Order Date is populated$/ do
  expect(test_param[:order_id][1]).to be_truthy
  5.times { break if stamps.orders.orders_grid.column.order_date.data(test_param[:order_id][1]).size > 4 }
  expect(stamps.orders.orders_grid.column.order_date.data(test_param[:order_id][1]).size).to be > 4
end
...
Then /^[Ee]xpect [Oo]rders [Gg]rid Recipient is (?:correct|(.*))$/ do |expectation|
  ...
  expect(stamps.orders.orders_grid.column.recipient.data(test_param[:order_id][1])).to eql expectation
end

Then /^[Ee]xpect [Oo]rders [Gg]rid Company is (?:correct|(.*))$/ do |expectation|
  expectation = test_param[:company] if expectation.nil?
  10.times { break if stamps.orders.orders_grid.column.company.data(test_param[:order_id][1]).eql? expectation }
  expect(stamps.orders.orders_grid.column.company.data(test_param[:order_id][1])).to eql expectation
end
...
Then /^[Ee]xpect [Oo]rders [Gg]rid Address is (?:correct|(.*))$/ do |expectation|
  ...
  10.times { break if stamps.orders.orders_grid.column.address.data(test_param[:order_id][1]).eql? expectation }
  expect(stamps.orders.orders_grid.column.address.data(test_param[:order_id][1])).to eql expectation
end

Then /^[Ee]xpect [Oo]rders [Gg]rid City is (?:correct|(.*))$/ do |expectation|
  expectation = test_param[:city] if expectation.nil?
  10.times { break if stamps.orders.orders_grid.column.city.data(test_param[:order_id][1]).eql? expectation }
  expect(stamps.orders.orders_grid.column.city.data(test_param[:order_id][1])).to eql expectation
end

Then /^[Ee]xpect [Oo]rders [Gg]rid State is (?:correct|(.*))$/ do |expectation|
  expectation = test_param[:state] if expectation.nil?
  10.times { break if stamps.orders.orders_grid.column.state.data(test_param[:order_id][1]).eql? expectation }
  expect(stamps.orders.orders_grid.column.state.data(test_param[:order_id][1])).to eql expectation
end

Then /^[Ee]xpect [Oo]rders [Gg]rid Zip is (?:correct|(.*))$/ do |expectation|
  expectation = test_param[:zip] if expectation.nil?
  10.times { break if stamps.orders.orders_grid.column.zip.data(test_param[:order_id][1]).eql? expectation }
  expect(stamps.orders.orders_grid.column.zip.data(test_param[:order_id][1])).to include(expectation)
end

Then /^[Ee]xpect [Oo]rders [Gg]rid Country is (?:correct|(.*))$/ do |expectation|
  expectation = test_param[:country] if expectation.nil?
  10.times { break if stamps.orders.orders_grid.column.country.data(test_param[:order_id][1]).eql? expectation }
  expect(stamps.orders.orders_grid.column.country.data(test_param[:order_id][1])).to include(expectation)
end

Then /^[Ee]xpect [Oo]rders [Gg]rid Email is (?:correct|(.*))$/ do |expectation|
  expectation = test_param[:email] if expectation.nil?
  10.times { break if stamps.orders.orders_grid.column.email.data(test_param[:order_id][1]).eql? expectation }
  expect(stamps.orders.orders_grid.column.email.data(test_param[:order_id][1])).to eql expectation
end

Then /^[Ee]xpect [Oo]rders [Gg]rid Phone is (?:correct|(.*))$/ do |expectation|
  expectation = test_param[:phone] if expectation.nil?
  10.times { break if stamps.orders.orders_grid.column.phone.data(test_param[:order_id][1]).eql? expectation }
  expect(stamps.orders.orders_grid.column.phone.data(test_param[:order_id][1])).to eql expectation
end

Then /^[Ee]xpect [Oo]rders [Gg]rid Pounds is (?:correct|(.*))$/ do |expectation|
  expectation = test_param[:pounds] if expectation.nil?
  20.times { break if stamps.orders.orders_grid.column.weight.lb(test_param[:order_id][1]).eql? expectation }
  expect(stamps.orders.orders_grid.column.weight.lb(test_param[:order_id][1])).to eql expectation.to_i
end

Then /^[Ee]xpect [Oo]rders [Gg]rid Ounces is (?:correct|(.*))$/ do |expectation|
  expectation = test_param[:ounces] if expectation.nil?
  10.times { break if stamps.orders.orders_grid.column.weight.oz(test_param[:order_id][1]).eql? expectation }
  expect(stamps.orders.orders_grid.column.weight.oz(test_param[:order_id][1])).to eql expectation.to_i
end

Then /^[Ee]xpect [Oo]rders [Gg]rid Weight is (\d+) lb. (\d+) oz.$/ do |pounds, ounces|
  expectation = "#{pounds} lbs. #{ounces} oz."
  10.times { break if stamps.orders.orders_grid.column.weight.data(test_param[:order_id][1]).eql? expectation }
  expect(stamps.orders.orders_grid.column.weight.data(test_param[:order_id][1])).to eql expectation
end

Then /^[Ee]xpect [Oo]rders [Gg]rid Weight\(lb\) is (.*)$/ do |expectation|
  10.times { break if stamps.orders.orders_grid.column.weight.lb(test_param[:order_id][1]).eql? expectation }
  expect(stamps.orders.orders_grid.column.weight.lb(test_param[:order_id][1])).to eql expectation
end

Then /^[Ee]xpect [Oo]rders [Gg]rid Weight\(oz\) is (.*)$/ do |expectation|
  10.times { break if stamps.orders.orders_grid.column.weight.oz(test_param[:order_id][1]).eql? expectation }
  expect(stamps.orders.orders_grid.column.weight.oz(test_param[:order_id][1])).to eql expectation
end

Then /^[Ee]xpect [Oo]rders [Gg]rid Qty. is (.+)$/ do |expectation|
  20.times { break if stamps.orders.orders_grid.column.qty.data(test_param[:order_id][1]).eql? expectation }
  expect(stamps.orders.orders_grid.column.qty.data(test_param[:order_id][1])).to eql expectation.to_i
end

Then /^[Ee]xpect [Oo]rders [Gg]rid Item SKU is (.+)$/ do |expectation|
  10.times { break if stamps.orders.orders_grid.column.item_sku.data(test_param[:order_id][1]).eql? expectation }
  expect(stamps.orders.orders_grid.column.item_sku.data(test_param[:order_id][1])).to eql expectation
end

Then /^[Ee]xpect [Oo]rders [Gg]rid Item Name is (.+)$/ do |expectation|
  10.times { break if stamps.orders.orders_grid.column.item_name.data(test_param[:order_id][1]).eql? expectation }
  expect(stamps.orders.orders_grid.column.item_name.data(test_param[:order_id][1])).to eql expectation
end

Then /^[Ee]xpect [Oo]rders [Gg]rid Ship From is (.+)$/ do |expectation|
  10.times { break if stamps.orders.orders_grid.column.ship_from.data(test_param[:order_id][1]).eql? expectation }
  expect(stamps.orders.orders_grid.column.ship_from.data(test_param[:order_id][1])).to eql expectation
end
...
Then /^[Ee]xpect [Oo]rders [Gg]rid Insured Value is \$(.+)$/ do |expectation|
  10.times { break if stamps.orders.orders_grid.column.insured_value.data(test_param[:order_id][1]).eql? expectation }
  expect(stamps.orders.orders_grid.column.insured_value.data(test_param[:order_id][1])).to eql expectation.to_f.round(2)
end

Then /^[Ee]xpect [Oo]rders [Gg]rid Reference No. is (.+)$/ do |expectation|
  10.times { break if stamps.orders.orders_grid.column.reference_no.data(test_param[:order_id][1]).eql? expectation }
  expect(stamps.orders.orders_grid.column.reference_no.data(test_param[:order_id][1])).to eql expectation
end

Then /^[Ee]xpect [Oo]rders [Gg]rid Cost Code is (.+)$/ do |expectation|
  10.times { break if stamps.orders.orders_grid.column.cost_code.data(test_param[:order_id][1]).eql? expectation }
  expect(stamps.orders.orders_grid.column.cost_code.data(test_param[:order_id][1])).to eql expectation
end

Then /^[Ee]xpect [Oo]rders [Gg]rid Tracking service is (.+)$/ do |expectation|
  10.times { break if stamps.orders.orders_grid.column.tracking_service.data(test_param[:order_id][1]) == expectation }
  expect(stamps.orders.orders_grid.column.tracking_service.data(test_param[:order_id][1])).to eql expectation
end

Then /^[Ee]xpect [Oo]rders [Gg]rid Order Status is (.+)$/ do |expectation|
  10.times { break if stamps.orders.orders_grid.column.order_status.data(test_param[:order_id][1]) == expectation }
  expect(stamps.orders.orders_grid.column.order_status.data(test_param[:order_id][1])).to eql expectation
end

Then /^[Ee]xpect [Oo]rders [Gg]rid Tracking Number is populated$/ do
  20.times { break if stamps.orders.orders_grid.column.tracking_no.data(test_param[:order_id][1]).length > 3 }
  expect(stamps.orders.orders_grid.column.tracking_no.data(test_param[:order_id][1]).length).to be > 3
end

Then /^[Ee]xpect [Oo]rders [Gg]rid Order Total is (.+)$/ do |expectation|
  10.times { break if stamps.orders.orders_grid.column.order_total.data(test_param[:order_id][1]).eql? expectation }
  expect(stamps.orders.orders_grid.column.order_total.data(test_param[:order_id][1])).to eql expectation
end
