...
Then /^(?:O|o)n Order Details form, expect Ship-From saved value is the same$/ do
...
Then /^(?:O|o)n Order Details form, expect International Ship-To Province is (.*)/ do |value|
...
Then /^(?:O|o)n Order Details form, expect Reference Number is (.*)$/ do |expectation|
...
Then /^(?:O|o)n Order Details form, expect Tracking Cost saved value is correct$/ do
  step "On Order Details form, expect Tracking Cost is $#{test_data[:tracking_cost]}"
end

Then /^(?:O|o)n Order Details form, expect Tracking Cost is \$([0-9.]*)$/ do |expectation|
  expectation = expectation.to_f
  10.times do
...
Then /^(?:O|o)n Order Details form, expect Pounds? is (.*)$/ do |expectation|
  expectation = expectation.to_f
  10.times do
...
Then /^(?:O|o)n Order Details form, expect Ounces? is (.*)$/ do |expectation|
  expectation = expectation.to_f
  10.times do
