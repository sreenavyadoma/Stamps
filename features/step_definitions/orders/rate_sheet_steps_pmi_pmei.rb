...
Then /^[Rr]un rate sheet (.*) in Country Price Group (\d+)$/ do |param_sheet, group|
  ...
  @rate_sheet.each_with_index do |row, row_number|
    ...
    begin
      if row_number > 0
        SdcLogger.step "#{"#" * 80} Rate Sheet: #{param_sheet}: Group #{group} - Row #{row_number}"
        ...
        if row[group_column] == nil
          ...
          # Set weight to 0
          SdcLogger.step "#{"#" * 10} Desired Weight: #{row[@rate_sheet_columns[:weight_lb]]}"
          ...
          SdcLogger.step "#{"#" * 10} "
          SdcLogger.step "#{"#" * 10} Weight: #{weight_lb}"
          SdcLogger.step "#{"#" * 10} Price: #{TestData.store[:result_sheet][row_number, TestData.store[:result_sheet_columns][:group]]}"
          SdcLogger.step "#{"#" * 10} "
          SdcLogger.step "#{"#" * 50}"
          ...
          SdcLogger.step "#{"#" * 10} Desired Service: #{service}"
          ...
          SdcLogger.step "#{"#" * 10} "
          SdcLogger.step "#{"#" * 10} Selected Weight: #{TestData.store[:result_sheet][row_number, TestData.store[:result_sheet_columns][:weight]]}"
          ...
          SdcLogger.step "#{"#" * 10} Selected Country: #{TestData.store[:country]}"
          SdcLogger.step "#{"#" * 10} #{"*" * 5} Test #{TestData.store[:result_sheet][row_number, TestData.store[:result_sheet_columns][:status]] } - Expected #{TestData.store[:result_sheet][row_number, TestData.store[:result_sheet_columns][:group]]}, Got #{TestData.store[:result_sheet][row_number, TestData.store[:result_sheet_columns][:total_ship_cost]]} #{"*" * 5}"
          SdcLogger.step "#{"#" * 10} "
        end
    ...
    begin
      if row_number > 0
        if row[TestData.store[:result_sheet_columns][:status]].casecmp("failed").zero? || (row[TestData.store[:result_sheet_columns][:status]].casecmp("passed") != 0 && !row[TestData.store[:result_sheet_columns][:error_msg]].nil?)
          @failed_test_count += 1
          SdcLogger.step "Group #{group} - Row #{row_number} Failed"
        end
      end
    end
  end
