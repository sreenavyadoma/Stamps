...
Then /^[Ii]n Orders Toolbar, expect number of orders on page is correct$/ do
  ...
  #SdcLog.step "Test #{(max_order_count==multi_order_count)?"Passed":"Failed"}"
...
Then /^Pagination control to go to first page is (\w+)$/ do |first_pagination_enabled|
  ...
  if first_pagination_enabled.downcase=="enabled"
    #SdcLog.step "Pagination control to go to first page is enabled.  Test #{(enabled)?'Passed':'Failed'}"
...
Then /^Pagination control to go to previous page is (\w+)$/ do |previous_pagination_enabled|
  ...
  if previous_pagination_enabled.downcase=="enabled"
    #SdcLog.step "Pagination control to go to previous page is enabled.  Test #{(enabled)?'Passed':'Failed'}"
    expect(enabled).to be(true)
  end
end
...
Then /^Pagination control to go to page number is (\w+$)$/ do |page_number_enabled|
  ...
  if page_number_enabled.downcase=="enabled"
    #SdcLog.step "Pagination control to go to page number is enabled.  Test #{(enabled)?'Passed':'Failed'}"
    expect(enabled).to be(true)
  end
end

Then /^Pagination control to go to next page is (\w+)$/ do |next_pagination_enabled|
  #SdcLog.step "Pagination control to go to next page is #{next_pagination_enabled}"
  enabled=stamps.orders.orders_toolbar.next_page.present?
  if next_pagination_enabled.downcase=="enabled"
    #SdcLog.step "Pagination control to go to next page is enabled.  Test #{(enabled)?'Passed':'Failed'}"
    expect(enabled).to be(true)
  end
end

Then /^Pagination control to go to last page is (\w+)$/ do |last_pagination_enabled|
  #SdcLog.step "Pagination control to go to last page is #{last_pagination_enabled}"
  enabled=stamps.orders.orders_toolbar.last_page.present?
  if last_pagination_enabled.downcase=="enabled"
    #SdcLog.step "Pagination control to go to last page is enabled.  Test #{(enabled)?'Passed':'Failed'}"
    expect(enabled).to be(true)
  end
end
...
When /^[Ss]et Page Number to (\d*)$/ do |value|
  ...
  begin
    #SdcLog.step "Set Page Number to \"#{value}\""
    ...
    #SdcLog.step "Current page number #{page_number}"
    ...
    textbox_field.click
    textbox_field.click
    #SdcLog.step "New page number #{page_number}"
...
Then /^Paging: Expect Total Number of Pages is (\d+)$/ do |total_number_of_pages|
  #SdcLog.step "Paging: Expect Total Number of Pages is #{total_number_of_pages}"
  #SdcLog.step "Passed value is #{total_number_of_pages}"
  browser_total_number_of_pages=stamps.orders.orders_toolbar.total_number_of_pages.to_s
  #SdcLog.step "DD value is #{browser_total_number_of_pages}"
  test_result=browser_total_number_of_pages.include? total_number_of_pages
  #SdcLog.step "#{(test_result)?'Test Passed.':'Test Failed'}"
...
Then /^Web Batch grid has more orders than value selected in 'number of orders per page' pagination control$/ do
  #SdcLog.step "Web Batch grid has more orders than value selected in 'number of orders per page' pagination control"
