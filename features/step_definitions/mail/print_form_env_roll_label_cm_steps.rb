...
Then /^[Ss]et Print form [Mm]ail-[Tt]o (?:|to )(?:|a )(?:|random )address(?: to| in| between|) (.*)$/ do |address|
  stamps.mail.print_form.mail_to.mail_address.textarea.set(test_param[:address]=TestHelper.address_helper(address))
end
...
Then /^[Ss]et Print form Length to (\d+)$/ do |length|
  stamps.mail.print_form.dimensions.length.set(test_param[:length]=length)
end

Then /^[Ss]et Print form width to (\d+)$/ do |width|
  stamps.mail.print_form.dimensions.width.set(test_param[:width]=width)
end

Then /^[Ss]et Print form height to (\d+)$/ do |height|
  stamps.mail.print_form.dimensions.height.set(test_param[:height]=height)
end