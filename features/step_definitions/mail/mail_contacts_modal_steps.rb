...
Then /^[Ss]earch Contacts Modal for last printed contact/ do
  ...
  pending #stamps.mail.print_form.mail_to.mail_to_link.click.search address_array[0]
end

Then /^[Ss]earch Contacts Modal for (.*)$/ do |str|
  pending #stamps.mail.print_form.mail_to.mail_to_link.click.search str
end

Then /^[Cc]lick Search Contacts close [Bb]utton$/ do
  pending #stamps.mail.print_form.mail_to.mail_to_link.click.close
end

Then /^[Ee]xpect Contact Name (.*) is present$/ do |str|
  pending #expect(stamps.mail.print_form.mail_to.mail_to_link.click.contact_name str).to be_present
end

Then /^[Ss]elect Contact Name (.*)$/ do |str|
  pending #stamps.mail.print_form.mail_to.mail_to_link.click.select str
end
...
Then /^[Ss]elect Recently Printed Contact$/ do
  ...
  pending #stamps.mail.print_form.mail_to.mail_to_link.click.select address_name_array_reversed
end
