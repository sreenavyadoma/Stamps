Then /^hover on (.*) column on contacts grid$/ do |column|
  column_header =SdcContacts::Grid::GridColumnBase.contacts_header_element(column)
...
Then /^click on (.*) column header dropdown trigger$/ do |column|
  column_trigger = SdcContacts::Grid::GridColumnBase.select_header_column_trigger(column)
...
Then /^expect contacts header dropdown menu is displayed$/ do
  menu_list=SdcContacts::Grid::GridColumnBase.header_dropdown_menu_list
  expect(menu_list.present?).to be(true)
end

Then /^expect (.*) is available in the contacts header dropdown menu list$/ do |menu_name|
  menu_item = SdcContacts::Grid::GridColumnBase.header_dropdown_menu_item(menu_name)
  menu_item.flash
