...
Then /^[Ss]et [Pp]rofile [Pp]age [Ee]mail to (?:random value|(.*))$/ do |str|
  ...
  registration.profile.email.textbox.set(TestData.store[:email]=(str.nil?)?(TestHelper.rand_email(modal_param.test_env)):str)
  step "blur out on profile page"
end

Then /^[Ss]et [Pp]rofile [Pp]age [Uu]sername to (?:random value|(.*))$/ do |str|
  registration.profile.account_username.textbox.set((TestData.store[:username]=(str.nil?)?(TestHelper.rand_usr(modal_param.test_env) ):str))
  step "blur out on profile page"
end

Then /^[Ee]xpect [Pp]rofile [Pp]age [Uu]sername is (?:correct|(.*))$/ do |str|
  expect(registration.profile.account_username.textbox.text).to eql((str.nil?)?TestData.store[:username]:str)
...
Then /^[Ss]et [Pp]rofile [Pp]age [Pp]assword to (?:random value|(.*))$/ do |str|
 # registration.profile.account_password.set(TestData.store[:password]=(str.nil?)?"pass111":str) #TestHelper.random_password
   registration.profile.account_password.textbox.set(TestData.store[:account_password]=(str.nil?)?TestHelper.rand_alpha_numeric(5, 13):str)
end

Then /^[Ee]xpect [Pp]rofile [Pp]age [Pp]assword is (?:correct|(.*))$/ do |str|
  expect(registration.profile.account_password.textbox.text).to eql((str.nil?)?TestData.store[:account_password]:str)
end

Then /^[Ss]et [Pp]rofile [Pp]age [Rr]e-[Tt]ype [Pp]assword to (?:same as previous password|(.*))$/ do |str|
  registration.profile.retype_password.textbox.set(TestData.store[:retype_password]=(str.nil?)?(TestData.store[:account_password]):str)

end

Then /^[Ee]xpect [Pp]rofile [Pp]age [Rr]e-[Tt]ype [Pp]assword is (?:correct|(.*))$/ do |str|
  expect(registration.profile.retype_password.textbox.text).to eql((str.nil?)?TestData.store[:retype_password]:str)
end
...
Then /^[Ss]et [Pp]rofile [Pp]age [Ss]urvey [Qq]uestion to (.*)$/ do |str|
  TestData.store[:survey_question_selected]=registration.profile.survey_question.survey_question_dropdown.select(TestData.store[:survey_question]=str)
  end

Then /^[Ee]xpect [Pp]rofile [Pp]age [Ss]urvey [Qq]uestion is (?:correct|(.*))$/ do |str|
  expect(registration.profile.survey_question.survey_question_dropdown.text).to include((str.nil?)?TestData.store[:survey_question]:str)
end
...
Then /^[Ss]et How did you hear about us\? to (.*)$/ do |str|
  TestData.store[:survey_question_selected]=registration.profile.referer_name.referer_name_dropdown.select(TestData.store[:referer_name]=str) if registration.profile.referer_name.referer_name_dropdown.present?
end

Then /^[Ee]xpect How did you hear about us\? is (?:correct|(.*))$/ do |str|
  expect(registration.profile.referer_name.referer_name_dropdown.text).to eql((str.nil?)?TestData.store[:referer_name]:str) if registration.profile.referer_name.present?
end

Then /^[Ss]et [Pp]rofile [Pp]age [Pp]romo [Cc]ode to (?:an empty string|(.*))$/ do |str|
  step "show profile page promo code textbox"
  registration.profile.promo_code.textbox.set(TestData.store[:promo_code]=(str.nil?)?'':str)
...
Then /^[Ee]xpect [Pp]rofile [Pp]age [Pp]romo [Cc]ode is (?:correct|(.*))$/ do |str|
  step "show profile page promo code textbox"
  expect(registration.profile.promo_code.textbox).to eql((str.nil?)?TestData.store[:promo_code]:str)
end
