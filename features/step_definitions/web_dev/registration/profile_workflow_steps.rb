...
Then /^[Ss]et [Pp]rofile [Pp]age [Ee]mail to (?:random value|(.*))$/ do |str|
  ...
  registration.profile.email.textbox.set(test_param[:email]=(str.nil?)?(TestHelper.rand_email(modal_param.test_env)):str)
  step "blur out on profile page"
end

Then /^[Ss]et [Pp]rofile [Pp]age [Uu]sername to (?:random value|(.*))$/ do |str|
  registration.profile.account_username.textbox.set((test_param[:username]=(str.nil?)?(TestHelper.rand_usr(modal_param.test_env) ):str))
  step "blur out on profile page"
end
...
Then /^[Ss]et [Pp]rofile [Pp]age [Pp]assword to (?:random value|(.*))$/ do |str|
 # registration.profile.account_password.set(test_param[:password]=(str.nil?)?"pass111":str) #TestHelper.random_password
