...
Then /^[Pp]P: [Ee]xpect [Hh]amburger button exists for browser$/ do
  PartnerPortal.pp_common_page.panel_hamburger.wait_until_present(timeout: 10)
  expect(PartnerPortal.pp_common_page.panel_hamburger).to be_present, 'Hamburger button DOES NOT exist browser mode'
end
...
Then /^[Pp]P: [Ee]xpect left panel to expand for browser$/ do
  PartnerPortal.pp_common_page.panel_expanded_lg.wait_until_present(timeout: 10)
  expect(PartnerPortal.pp_common_page.panel_expanded_lg).to be_present, 'Expanded Panel DOES NOT exist browser mode'
end

Then /^[Pp]P: [Ee]xpect left panel to collapse for browser$/ do
  PartnerPortal.pp_common_page.panel_collapsed_lg.wait_until_present(timeout: 10)
  expect(PartnerPortal.pp_common_page.panel_collapsed_lg).to be_present, 'Collapsed Panel DOES NOT exist browser mode'
end
...
Then /^[Pp]P: [Ee]xpect arrow on left panel does not exists for browser$/ do
  expect(PartnerPortal.pp_common_page.panel_arrow).not_to be_present, 'Arrow on left panel is PRESENT mobile mode'
end

Then /^[Pp]P: [Ee]xpect arrow on left panel exists for browser$/ do
  PartnerPortal.pp_common_page.panel_arrow.wait_until_present(timeout: 10)
  expect(PartnerPortal.pp_common_page.panel_arrow).to be_present, 'Arrow on Panel DOES NOT exist browser mode'
end


Then /^[Pp]P: [Cc]lick on arrow on left panel$/ do
  PartnerPortal.pp_common_page.panel_arrow.wait_until_present(timeout: 10)
  PartnerPortal.pp_common_page.panel_arrow.click
end
