...
Then /^[Pp]P: [Aa] user navigates to the login page$/ do
  PartnerPortals.visit
end

Then /^[Pp]P: [Ee]xpect login page "Welcome to the" content to exist$/ do
  expect(PartnerPortals.login_page.welcome_content).to be_present, "Welcome to the content DOES NOT exist on login page"

end

Then /^[Pp]P: [Ee]xpect login page "stamps.com endicia" logo to exist$/ do
  expect(PartnerPortals.login_page.sdc_endicia_logo).to be_present, "stamps.com endicia content DOES NOT exist on login page"
end

Then /^[Pp]P: [Ee]xpect login page "USPS Portal" content to exist$/ do
  expect(PartnerPortals.login_page.usps_portal_content).to be_present, "USPS Portal content DOES NOT exist on login page"
end

Then /^[Pp]P: [Ee]xpect login page Log In button to exist$/ do
  expect(PartnerPortals.login_page.log_in).to be_present, "Log In button DOES NOT exist on login page"
end

Then /^[Pp]P: [Ee]xpect login page Log In button text is (.*)$/ do |str|
 expect(PartnerPortals.login_page.login_label.text_value).to eql(str)
end

Then /^[Pp]P: [Uu]ser clicks Log In$/ do
  PartnerPortals.login_page.log_in.safe_click
end

Then /^[Pp]P: expect login page [Ff]orgot [Pp]assword link exist$/ do
  expect(PartnerPortals.login_page.forgot_pw).to be_present, "Forgot Password? link DOES NOT exist on login page"
end

Then /^[Pp]P: [Uu]ser clicks [Ff]orgot [Pp]assword link$/ do
  PartnerPortals.login_page.forgot_pw.safe_click
end

Then /^[Pp]P: [Bb]lur out on [Ll]ogin [Pp]age$/ do
  PartnerPortals.login_page.usps_portal_content.blur_out
end


Then /^[Pp]P: [Ee]xpect login page [Ee]mail field to exist$/ do
  expect(PartnerPortals.login_page.email).to be_present, "Email textbox DOES NOT exist on login page"
end


Then /^[Pp]P: [Ee]xpect login page [Ee]mail field shows placeholder (.*)$/ do |str|
  expect(PartnerPortals.login_page.email_placeholder.attribute_value 'PLACEHOLDER').to eql(str)

end


Then /^[Pp]P: set login page email to (.*)$/ do |str|
  PartnerPortals.login_page.email.set(TestData.store[:email]=str)
end



Then /^[Pp]P: [Ee]xpect login page email tooltip count is (.*)$/ do |count|
  TestData.store[:email_tooltip] = PartnerPortals.login_page.email_tooltip.text_value.split("\n")
...
Then /^[Pp]P: [Ee]xpect login page Password field to exist$/ do
  expect(PartnerPortals.login_page.password).to be_present, "Password textbox DOES NOT exist on login page"
end

Then /^[Pp]P: set login page password to (.*)$/ do |str|
  PartnerPortals.login_page.password.set(TestData.store[:password]=str)
end


Then /^[Pp]P: [Ee]xpect login page [Pp]assword field shows placeholder (.*)$/ do |str|
  expect(PartnerPortals.login_page.password_placeholder.attribute_value 'PLACEHOLDER').to eql(str)

end

Then /^[Pp]P: [Ee]xpect login page password tooltip count is (.*)$/ do |count|
  TestData.store[:password_tooltip] = PartnerPortals.login_page.password_tooltip.text_value.split("\n")
...
Then /^[Pp]P: [Ee]xpect login page error message to be$/ do |str|
  expect(PartnerPortals.login_page.error_message.text).to eql(str)
end
