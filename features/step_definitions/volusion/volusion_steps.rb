...
Then /^Volusion: Visit Page$/ do
  step "I launched default driver" if SdcTest.driver.nil?
  volusion.visit
end
...
Then /^Volusion New to Mail Stamps: Set email address to (.*)$/ do |email|
  @volusion_email = (email.downcase == 'random') ? "#{TestHelper.rand_alpha_numeric}@mailinator.com" : email
...
Then /^Volusion Shipping Address: Set First Name to (.*)$/ do |value|
  @volusion_shipping_address.first_name.set(value.downcase == 'random') ? (TestHelper.rand_full_name) : value
end

Then /^Volusion Shipping Address: Set Last Name to (.*)$/ do |value|
  @volusion_shipping_address.last_name.set(value.downcase == 'random') ? (TestHelper.rand_full_name) : value
end

Then /^Volusion Shipping Address: Set Company to (.*)$/ do |value|
  @volusion_shipping_address.company.set(value.downcase == 'random') ? (TestHelper.rand_full_name) : value
end
...
Then /^Volusion Shipping Address: Set Phone Number to (.*)$/ do |value|
  @volusion_shipping_address.phone_number.set(value.downcase == 'random') ? (TestHelper.rand_phone) : value
end
