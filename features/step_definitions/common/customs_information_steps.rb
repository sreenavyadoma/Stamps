...
Then /^[Cc]lick (?:Order Details|Print) [Ff]orm (?:[Ee]dit [Cc]ustoms|[Ee]dit|[Cc]ustoms) [Ff]orm [Bb]utton$/ do
  stamps.orders.order_details.customs.edit_customs_form if modal_param.web_app==:orders
...
Then /^[Bb]lur [Oo]ut [Oo]n [Cc]ustoms [Ff]orm(?:| (\d+)(?:| times))$/ do |count|
  ...
  count.times { stamps.orders.order_details.customs.edit_customs_form.blur_out } if modal_param.web_app==:orders
...
Then /^[Ee]xpect Customs Non-Delivery Options is (?:correct|(.*))$/ do |expectation|
  ...
  10.times do
    if modal_param.web_app==:orders
      actual=stamps.orders.order_details.customs.edit_customs_form.non_delivery_options.textbox.text
  ...
  expect(stamps.orders.order_details.customs.edit_customs_form.non_delivery_options.textbox.text).to eql(expectation) if modal_param.web_app==:orders
...
Then /^[Dd]elete Customs form Associated Item (\d+)$/ do |item_number|
  if modal_param.web_app==:orders
    field=stamps.orders.order_details.customs.edit_customs_form.associated_items
  else
...
Then /^[Cc]heck Customs form I agree to the USPS Privacy Act Statement$/ do
  stamps.orders.order_details.customs.edit_customs_form.agree_to_terms.check if modal_param.web_app==:orders
...
Then /^[Uu]ncheck Customs form I agree to the USPS Privacy Act Statement$/ do
  stamps.orders.order_details.customs.edit_customs_form.agree_to_terms.uncheck if modal_param.web_app==:orders
...
Then /^[Cc]lose Customs Information form$/ do
  ...
  step "Save Customs Information form Total amount"
  if modal_param.web_app==:orders
    stamps.orders.order_details.customs.edit_customs_form.close
...
Then /^Cancel Customs Form$/ do
  ...
  expect(stamps.orders.order_details.customs.edit_customs_form.cancel).to be(false) if modal_param.web_app==:orders
...
Then /^[Ee]xpect Customs More Info is hidden$/ do
  ...
  expect(stamps.orders.order_details.customs.edit_customs_form.package_contents.contents.more_info).not_to be_present if modal_param.web_app==:orders
...
Then /^[Ss]ave Customs Information form [Tt]otal amount$/ do
  test_param[:customs_total_value]=stamps.orders.order_details.customs.edit_customs_form.total_value if modal_param.web_app==:orders
...
Then /^[Aa]dd Customs form Associated Item (\d+)$/ do |item_number|
  stamps.orders.order_details.customs.edit_customs_form.associated_items.item_number(item_number.to_i) if modal_param.web_app==:orders
