...
Then /^[Ss]et Customs form Package Contents to (.*)$/ do |value|
  parameter[:customs_package_contents] = value
  stamps.orders.order_details.customs.edit_form.package_contents.select(parameter[:customs_package_contents]) if param.web_app == :orders
  stamps.mail.print_form.mail_customs.edit_form.package_contents.select(parameter[:customs_package_contents]) if param.web_app == :mail
  step "Save Customs Information form Total amount"
end

Then /^[Ee]xpect Customs form Package Contents is (?:correct|(.*))$/ do |expectation|
  expectation = (expectation.nil?)?parameter[:customs_package_contents] : expectation
  sleep(0.15)
...
Then /^[Ss]et Customs form Non-Delivery Options to (.*)$/ do |value|
  parameter[:customs_non_delivery_options] = value
  stamps.orders.order_details.customs.edit_form.non_delivery_options.select(parameter[:customs_non_delivery_options]) if param.web_app == :orders
  stamps.mail.print_form.mail_customs.edit_form.non_delivery_options.select(parameter[:customs_non_delivery_options]) if param.web_app == :mail
end

Then /^[Ee]xpect Customs form Non-Delivery Options is (?:correct|(.*))$/ do |expectation|
  expectation = (expectation.nil?)?parameter[:customs_non_delivery_options] : expectation
...
Then /^[Ss]et Customs form Internal Transaction Number to (.*)$/ do |value|
  parameter[:customs_internal_transaction_no] = value
  stamps.orders.order_details.customs.edit_form.internal_transaction.select(parameter[:customs_internal_transaction_no]) if param.web_app == :orders
  stamps.mail.print_form.mail_customs.edit_form.internal_transaction.select(parameter[:customs_internal_transaction_no]) if param.web_app == :mail
  step "Save Customs Information form Total amount"
end

Then /^[Ee]xpect Customs form Internal Transaction Number is (?:correct|(.*))$/ do |expectation|
  expectation = (expectation.nil?)?parameter[:customs_internal_transaction_no] : expectation
  sleep(0.5)
...
Then /^[Ss]et Customs form More Info to (?:(?:a|some) random string|(.*))$/ do |value|
  parameter[:customs_more_info] = (value.nil?)?helper.random_alpha_numeric(18):value
  stamps.orders.order_details.customs.edit_form.package_contents.contents.more_info.set(parameter[:customs_more_info]) if param.web_app == :orders
  stamps.mail.print_form.mail_customs.edit_form.package_contents.contents.more_info.set(parameter[:customs_more_info]) if param.web_app == :mail
  step "Save Customs Information form Total amount"
end

Then /^[Ee]xpect Customs form More Info is (?:correct|(.*))$/ do |expectation|
  expectation = (expectation.nil?)?parameter[:customs_more_info] : expectation
  sleep(0.5)
...
Then /^[Ss]et Customs form ITN Number to (?:(?:a|some) random string|(.*))$/ do |value|
  parameter[:customs_itn_no] = (value.nil?)?helper.random_alpha_numeric(50):value
  stamps.orders.order_details.customs.edit_form.itn_number.set(parameter[:customs_itn_no]) if param.web_app == :orders
  stamps.mail.print_form.mail_customs.edit_form.itn_number.set(parameter[:customs_itn_no]) if param.web_app == :mail
  step "Save Customs Information form Total amount"
end

Then /^[Ee]xpect Customs form ITN Number is (?:correct|(.*))$/ do |expectation|
  expectation = (expectation.nil?)?parameter[:customs_itn_no] : expectation
  sleep(0.5)
...
Then /^[Ss]et Customs form License Number to (?:(?:a|some) random string|(.*))$/ do |value|
  parameter[:customs_license_no] = ((value.nil?)?helper.random_alpha_numeric(6): value)
  stamps.orders.order_details.customs.edit_form.package_contents.contents.license.set(parameter[:customs_license_no]) if param.web_app == :orders
  stamps.mail.print_form.mail_customs.edit_form.package_contents.contents.license.set(parameter[:customs_license_no]) if param.web_app == :mail
  step "Save Customs Information form Total amount"
end

Then /^[Ee]xpect Customs form License Number is (?:correct|(.*))$/ do |expectation|
  expectation = (expectation.nil?)?parameter[:customs_license_no] : expectation
  sleep(1)
...
Then /^[Ss]et Customs form Certificate Number to (?:(?:a|some) random string|(.*))$/ do |value|
  parameter[:customs_certificate_no] = ((value.nil?)?helper.random_alpha_numeric(8):value)
  stamps.orders.order_details.customs.edit_form.package_contents.contents.certificate.set(parameter[:customs_certificate_no]) if param.web_app == :orders
  stamps.mail.print_form.mail_customs.edit_form.package_contents.contents.certificate.set(parameter[:customs_certificate_no]) if param.web_app == :mail
  step "Save Customs Information form Total amount"
end

Then /^[Ee]xpect Customs form Certificate Number is (?:correct|(.*))$/ do |expectation|
  expectation = (expectation.nil?)?parameter[:customs_certificate_no]:expectation
  sleep(0.5)
...
Then /^[Ss]et Customs form Invoice Number to (?:(?:a|some) random string|(.*))$/ do |value|
  parameter[:customs_invoice_no] = (value.nil?)?helper.random_alpha_numeric(10):value
  stamps.orders.order_details.customs.edit_form.package_contents.contents.invoice.set(parameter[:customs_invoice_no]) if param.web_app == :orders
  stamps.mail.print_form.mail_customs.edit_form.package_contents.contents.invoice.set(parameter[:customs_invoice_no]) if param.web_app == :mail
  step "Save Customs Information form Total amount"
end

Then /^[Ee]xpect Customs form Invoice Number is (?:correct|(.*))$/ do |expectation|
  expectation = (expectation.nil?)?parameter[:customs_invoice_no]:expectation
  sleep(0.5)
...
Then /^[Ss]ave Customs Information form [Tt]otal amount$/ do
  parameter[:customs_total_value] = stamps.orders.order_details.customs.edit_form.total_value if param.web_app == :orders
  parameter[:customs_total_value] = stamps.mail.print_form.mail_customs.edit_form.total_value if param.web_app == :mail
end

Then /^[Ee]xpect Customs form Total Value is (?:correct|(.*))$/ do |expectation|
  expectation = (expectation.nil?)?parameter[:customs_total_value] : expectation.to_f.round(2)
  step "Blur out on Customs form"
...
Then /^[Ss]et Customs form Associated Item (\d+) Description to (.*)$/ do |item_number, value|
  parameter[:customs_associated_items][item_number] = Hash.new unless parameter[:customs_associated_items].has_key?(item_number)
  parameter[:customs_associated_items][item_number][:description] = (value.downcase.include?('random')?(helper.random_alpha_numeric):value)
  stamps.orders.order_details.customs.edit_form.associated_items.item_number(item_number.to_i).customs_item_description.set(parameter[:customs_associated_items][item_number][:description]) if param.web_app == :orders
  stamps.mail.print_form.mail_customs.edit_form.associated_items.item_number(item_number.to_i).customs_item_description.set(parameter[:customs_associated_items][item_number][:description]) if param.web_app == :mail
  step "Save Customs Information form Total amount"
end

Then /^[Ss]et Customs form Associated Item (\d+) Qty to (\d+)$/ do |item_number, value|
  parameter[:customs_associated_items][item_number] = Hash.new unless parameter[:customs_associated_items].has_key?(item_number)
  parameter[:customs_associated_items][item_number][:quantity] = value
  stamps.orders.order_details.customs.edit_form.associated_items.item_number(item_number.to_i).customs_item_qty.set(parameter[:customs_associated_items][item_number][:quantity]) if param.web_app == :orders
  stamps.mail.print_form.mail_customs.edit_form.associated_items.item_number(item_number.to_i).customs_item_qty.set(parameter[:customs_associated_items][item_number][:quantity]) if param.web_app == :mail
  step "Save Customs Information form Total amount"
end

Then /^[Ss]et Customs form Associated Item (\d+) Unit Price to (.*)$/ do |item_number, value|
  parameter[:customs_associated_items][item_number] = Hash.new unless parameter[:customs_associated_items].has_key?(item_number)
  parameter[:customs_associated_items][item_number][:price] = value
  stamps.orders.order_details.customs.edit_form.associated_items.item_number(item_number.to_i).customs_item_unit_price.set(parameter[:customs_associated_items][item_number][:price]) if param.web_app == :orders
  stamps.mail.print_form.mail_customs.edit_form.associated_items.item_number(item_number.to_i).customs_item_unit_price.set(parameter[:customs_associated_items][item_number][:price]) if param.web_app == :mail
  step "Save Customs Information form Total amount"
end

Then /^[Ss]et Customs form Associated Item (\d+) Made In is Country to (.*)$/ do |item_number, value|
  parameter[:customs_associated_items][item_number] = Hash.new unless parameter[:customs_associated_items].has_key?(item_number)
  parameter[:customs_associated_items][item_number][:made_in] = value
  stamps.orders.order_details.customs.edit_form.associated_items.item_number(item_number.to_i).made_in.select(parameter[:customs_associated_items][item_number][:made_in]) if param.web_app == :orders
  stamps.mail.print_form.mail_customs.edit_form.associated_items.item_number(item_number.to_i).made_in.select(parameter[:customs_associated_items][item_number][:made_in]) if param.web_app == :mail
end

Then /^[Ss]et Customs form Associated Item (\d+) Tarriff to (.*)$/ do |item_number, value|
  parameter[:customs_associated_items][item_number] = Hash.new unless parameter[:customs_associated_items].has_key?(item_number)
  parameter[:customs_associated_items][item_number][:tarriff] = value
  stamps.orders.order_details.customs.edit_form.associated_items.item_number(item_number.to_i).customs_item_hs_tariff.set(parameter[:customs_associated_items][item_number][:tarriff]) if param.web_app == :orders
  stamps.mail.print_form.mail_customs.edit_form.associated_items.item_number(item_number.to_i).customs_item_hs_tariff.set(parameter[:customs_associated_items][item_number][:tarriff]) if param.web_app == :mail
  step "Save Customs Information form Total amount"
end

Then /^[Ee]xpect Customs form Associated Item (\d+) Description is (?:correct|(.*))$/ do |item_number, expectation|
  expectation = (expectation.nil?)?parameter[:customs_associated_items][item_number][:description] : expectation
  sleep(0.5)
...
Then /^[Ee]xpect Customs form Associated Item (\d+) Quantity is (?:correct|(\d+))$/ do |item_number, expectation|
  expectation = (expectation.nil?)?parameter[:customs_associated_items][item_number][:quantity] : expectation
  sleep(0.5)
...
Then /^[Ee]xpect Customs form Associated Item (\d+) Unit Price is (?:correct|(.*))$/ do |item_number, expectation|
  expectation = (expectation.nil?)?parameter[:customs_associated_items][item_number][:price] :expectation
  sleep(0.5)
...
Then /^[Ee]xpect Customs form Associated Item (\d+) Made In is (?:correct|(.*))$/ do |item_number, expectation|
  expectation = (expectation.nil?)?parameter[:customs_associated_items][item_number][:made_in] :expectation
  sleep(0.5)
...
Then /^[Ee]xpect Customs form Associated Item (\d+) Tariff is (?:correct|(.*))$/ do |item_number, expectation|
  expectation = ((expectation.nil?)?parameter[:customs_associated_items][item_number][:tarriff] :expectation).to_f
  sleep(0.5)
