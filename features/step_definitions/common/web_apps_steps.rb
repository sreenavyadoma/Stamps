...
Then /^[Ss]ign-in to SDC Website$/ do
  ...
  expect(SdcWebsite.orders.landing_page.sign_in_with(TestData.store[:username], TestData.store[:password])).to eql(TestData.store[:username]) if SdcEnv.web_app == :orders
