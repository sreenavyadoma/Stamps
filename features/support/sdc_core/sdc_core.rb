  ...
  class SdcPage < WatirDrops::PageObject
    ...
    class << self
      ...
      def page_objs(name, tag: nil, index: nil, required: false, timeout: 30, &block)
        collection = index.nil? ? name : "#{name}s".to_sym
        _page_objects(collection) { SdcFinder.elements(browser, tag: tag, timeout: timeout, &block) }
        _page_object(name, required: required) { SdcElement.new(instance_eval(collection.to_s)[index]) } if index
      end
