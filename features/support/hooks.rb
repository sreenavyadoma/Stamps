...
Before do  |scenario|
  ...
  "").to eql "Environment variable URL is not defined!" if (ENV['URL'].nil? || ENV['URL'].size==0)
  logger.message "Test Name: #{ENV['USER_CREDENTIALS']}"
  "").to eql "Environment variable USER_CREDENTIALS is not defined!" if (ENV['USER_CREDENTIALS'].nil? || ENV['USER_CREDENTIALS'].size==0)
  logger.message "Browser: #{ENV['BROWSER']}"
  "").to eql "Environment variable BROWSER is not defined!" if (ENV['BROWSER'].nil? || ENV['BROWSER'].size==0)
  logger.message "-"
  logger.message "-"
  logger.message "Verbose: #{ENV['DEBUG']}"
  ENV['DEBUG']).to be_truthy
  logger.message "Healthcheck: #{ENV['HEALTHCHECK']}"
  ENV['HEALTHCHECK']).to be_truthy
  logger.message "-"
  logger.message "-"
  logger.message "-"
  ...
  begin
    if ENV['WEB_APP'].nil?
      "cucumber.yml: Missing WEB_APP variable").to eql "WEB_APP is nil"
      ...
      if (ENV['USR'].nil?) || (ENV['USR'].size==0) || (ENV['USR'].downcase == 'default') || (ENV['USR'].downcase == 'jenkins')
        ...
        begin
          if ENV['WEB_APP'].downcase == 'orders'
            ...
            "Valid WEB_APP values are orders and mail. You may add to the list, see hooks.rb").to eql "Invalid WEB_APP selection. #{ENV['WEB_APP']} is not recognized."
          end
        rescue => e
          if e.message.include? "mapping values are not allowed"
            "Formatting issues in default.yml file").to eql "default.yml - #{e.message.split(':').last}}"
          else
        ...
        begin
          ...
          "Missing credentials in #{ENV['WEB_APP']} Parameter credentials #{ENV['URL']}:#{ENV['USER_CREDENTIALS']}").to eql "There are no user credentials defined in default.yml file for URL:#{ENV['URL']} USER_CREDENTIALS:#{ENV['USER_CREDENTIALS']} usr:#{ENV['usr']} - #{e.message}"
        end
      ...
      "Valid values are WEB_APP=orders or WEB_APP=mail").to eql "WEB_APP=#{ENV['WEB_APP']} is not a valid value."
    end
