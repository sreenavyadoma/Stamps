    ...
    module PrintFormPanel
      class MailExtraServices < Browser::StampsBrowserElement
        ...
        def initialize(param)
          ...
          @value = StampsNumberField.new(param, text_box, inc_btn, dec_btn, 'Value')
        ...
        def close
          ...
          expect(close_element.present?).to be(false)
        end

        def save
          save_element.click_while_present
          expect(save_element.present?).to be(false)
        end

        def security_price
          expect(security_price_element.present?).to be(true)
          security_price_element.blur_out(3)
          ParameterHelper.remove_dollar_sign(security_price_element.text).to_f.round(2)
        end

        def return_receipt_price
          expect(return_receipt_price_element.present?).to be(true)
          return_receipt_price_element.blur_out(3)
          ParameterHelper.remove_dollar_sign(return_receipt_price_element.text).to_f.round(2)
        end
        ...
        def restricted_delivery_price
          ...
          ParameterHelper.remove_dollar_sign(restricted_delivery_price_element.text).to_f.round(2)
        end

        def cod_price
          expect(cod_price_element.present?).to be(true)
          cod_price_element.blur_out(3)
          ParameterHelper.remove_dollar_sign(cod_price_element.text).to_f.round(2)
        end
        ...
        def non_delivery_notice_price
          ...
          ParameterHelper.remove_dollar_sign(non_delivery_notice_price_element.text).to_f.round(2)
        end

        def content_price
          expect(content_price_element.present?).to be(true)
          content_price_element.blur_out(3)
          ParameterHelper.remove_dollar_sign(content_price_element.text).to_f.round(2)
        end
        ...
        def special_handling_price
          ...
          ParameterHelper.remove_dollar_sign(special_handling_price_element.text).to_f.round(2)
        end
        ...
        def merchandise_return_receipt
          ...
          ParameterHelper.remove_dollar_sign(merchandise_return_receipt_element.text).to_f.round(2)
        end

        def total_price
          expect(total_price_element.present?).to be(true)
          total_price_element.blur_out(3)
          ParameterHelper.remove_dollar_sign(total_price_element.text).to_f.round(2)
        end
        ...
        def cod
          if @cod.nil? || !@cod.present?
            ...
            @cod = StampsNumberField.new(param, text_box, inc_btn, dec_btn, 'Value')
          end
          @cod
        end
