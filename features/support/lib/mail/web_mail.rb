        ...
        case param.print_media
          when :stamps
            if cache[:stamps].nil? || !cache[:stamps].present?
              cache[:stamps] = Class.new(Browser::BaseCache).new(param).extend(PrintFormPanel::MailStamps)
            end
            return cache[:stamps]
          when :label
            if cache[:label].nil? || !cache[:label].present?
              cache[:label] = Class.new(Browser::BaseCache).new(param).extend(PrintFormPanel::ShippingLabel)
            end
            return cache[:label]
          when :envelope
            if cache[:envelope].nil? || !cache[:envelope].present?
              cache[:envelope] = Class.new(Browser::BaseCache).new(param).extend(PrintFormPanel::Envelope)
            end
            return cache[:envelope]
          when :certified_mail
            if cache[:cm].nil? || !cache[:cm].present?
              cache[:cm] = Class.new(Browser::BaseCache).new(param).extend(PrintFormPanel::CertifiedMail)
            end
            return cache[:cm]
          when :certified_mail_3910_3930
            if cache[:cm39103930].nil? || !cache[:cm39103930].present?
              cache[:cm39103930] = Class.new(Browser::BaseCache).new(param).extend(PrintFormPanel::CertifiedMail39103930)
            end
            return cache[:cm39103930]
          when :certified_mail_3810
            if cache[:cm3810].nil? || !cache[:cm3810].present?
              cache[:cm3810] = Class.new(Browser::BaseCache).new(param).extend(PrintFormPanel::CertifiedMail3810)
            end
            return cache[:cm3810]
          when :certified_mail_3830
            if cache[:certified_mail_3830_print_form].nil? || !cache[:certified_mail_3830_print_form].present?
              cache[:certified_mail_3830_print_form] = Class.new(Browser::BaseCache).new(param).extend(PrintFormPanel::CertifiedMail3830)
            end
            return cache[:certified_mail_3830_print_form]
          when :roll
            if cache[:roll].nil? || !cache[:roll].present?
              cache[:roll] = Class.new(Browser::BaseCache).new(param).extend(PrintFormPanel::Roll)
            end
            return cache[:roll]
