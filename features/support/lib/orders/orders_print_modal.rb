  ...
  module Orders
    module Printing
      class OrdersPrintMediaDropList < WebApps::Base
        ...
        def textbox
          cache[:printing_on].nil? || !cache[:printing_on].present? ? cache[:printing_on] = StampsTextbox.new(driver.text_field(css: "input[name^=printmediadroplist]")) : cache[:printing_on]
        end
        ...
        def selection_field(media)
          case(media)
            when /Paper/ # Shipping Label: 8 ½" x 11" Paper
              return StampsField.new(driver.li visible_text: /Paper/)
            when /SDC-1200/ # Shipping Label: Stamps.com SDC-1200, 4 ¼" x 6 ¾"
              return StampsField.new(driver.li visible_text: /SDC-1200/)
            when /Shipping Label - 5 / # Shipping Label: 5 ½" x 8 ½"
              return StampsField.new(driver.li visible_text: /Shipping Label - 5 /)
            when /Roll - 418x614/ # Roll 418x614
              return StampsField.new(driver.li visible_text: /Roll - 4 ⅛/)
      ...
      class OrdersPrinter < WebApps::Base
        ...
        def textbox
          @textbox.nil? || !@textbox.present? ? @textbox = StampsTextbox.new(driver.text_field(id: "sdc-printpostagewindow-printerdroplist-inputEl")) : @textbox
        end
      ...
      class OrdersPaperTray < WebApps::Base
        ...
        def textbox
          @textbox.nil? || !@textbox.present? ? @textbox = StampsTextbox.new(driver.text_field(name: "paperTrays")) : @textbox
        end
      ...
      class OrdersStartingLabel < WebApps::Base
        def left_label
          @left_label.nil? || !@left_label.present? ? @left_label = StampsField.new(driver.div(css: "div[class*=label-chooser-container-border]:nth-child(2)>div>div>div:nth-child(1)")) : @left_label
        end

        def right_label
          @right_label.nil? || !@right_label.present? ? @right_label = StampsField.new(driver.div(css: "div[class*=label-chooser-container-border]:nth-child(2)>div>div>div:nth-child(2)")) : @right_label
        end
      ...
      class LabelUnavailable < WebApps::Base
        def present?
          driver.div(text: "Label Unavailable").present?
        end

        def close
          button = StampsField.new driver.img(css: "img[class*='x-tool-img x-tool-close']")
          10.times do
            button.click
        ...
        def message
          StampsField.new(driver.divs(css: "div[id^=dialoguemodal][id$=innerCt]").last).text
        end
      end
      ...
      class OrdersUspsTerms < WebApps::Base
        ...
        def initialize(param)
          super
          @agree_to_terms = StampsField.new driver.span text: "I Agree"
          @cancel = StampsField.new driver.span text: "Cancel"
        end

        def present?
          (StampsField.new driver.div text: "USPS Terms").present?
        end
        ...
        def dont_show_this_again dont_show
          checkbox_field = (driver.inputs css: "input[id^=checkbox-][id$=-inputEl]").last
      ...
      class OrdersDatePicker < WebApps::Base
        ...
        def todays_date_div
          driver.div(css: "div[title='Today']")
        end

        def today_field
          StampsField.new(driver.div(css: "div[title='Today']"))
        end

        def date_field(day)
          driver.td(css: "td[aria-label='#{TestHelper.this_month_plus day.to_i}']")
        end
        ...
        def now_month_dd
          picker = StampsField.new(driver.div(css: "div[id^=datefield][id$=trigger-picker]"))
          today = StampsField.new(driver.span css: "a[title*=Spacebar]>span>span>span[data-ref=btnInnerEl]")
          10.times {
        ...
        def todays_date
          picker = StampsField.new driver.div(css: "div[id^=datefield][id$=trigger-picker]")
          today = StampsField.new StampsField.new driver.div css: "div[title=Today]"
          10.times {
            picker.click unless today.present?
            today.click
            sleep(0.35)
        ...
        def today_plus(day=0)
          ...
          date_picker_header = StampsField.new(driver.div(class: "x-datepicker-header"))
          picker_button = StampsField.new driver.div(css: "div[id^=datefield][id$=trigger-picker]")
          ship_date_textbox = StampsTextbox.new(driver.text_field(css: "input[id^=datefield][id$=inputEl]"))

          ship_date_str = TestHelper.this_month_plus(day)
          ship_date_mmddyy = TestHelper.today_plus(day)
          date_field = StampsField.new(driver.div(css: "td[aria-label='#{ship_date_str}']>div"))
      ...
      class OrdersShipDate < WebApps::Base
        ...
        def shipdate_label
          cache[:shipdate_label].nil? || !cache[:shipdate_label].present? ? cache[:shipdate_label] = StampsField.new(driver.span(css: '[id=sdc-printpostagewindow-shipdate-targetEl] [class*=x-form-item-label-inner-default]')) : cache[:shipdate_label]
        end
        ...
        def textbox
          cache[:textbox].nil? || !cache[:textbox].present? ? cache[:textbox] = StampsTextbox.new(driver.text_field(css: "[id=sdc-printpostagewindow-shipdate-innerCt] input[id^=datefield]")) : cache[:textbox]
        end
      ...
      module OrdersPrintModalFooter
        ...
        def total_cost
          StampsField.new(driver.label(text: 'Total Cost:').parent.labels.last).text.dollar_amount_str.to_f.round(2)
        end
        ...
        def print_sample_button
          cache[:print_sample_button].nil? || !cache[:print_sample_button].present? ? cache[:print_sample_button] = StampsField.new(driver.span(id: 'sdc-printwin-printsamplebtn-btnInnerEl')) : cache[:print_sample_button]
        end

        def print_button
          cache[:print_button].nil? || !cache[:print_button].present? ? cache[:print_button] = StampsField.new(driver.span(id: 'sdc-printwin-printbtn-btnInnerEl')) : cache[:print_button]
        end
      end

      module OrdersPrintModalTitle
        def window_title
          StampsField.new(driver.div(css: '[id^=printwindow-][id$=_header-innerCt] [class*=x-title-text-default]'))
        end
        ...
        def x_image
          StampsField.new(driver.img(css: "[id^=printwindow-] [class*=close]"))
        end
      ...
      class RePrintModal < OrdersPrintModal
        def present?
          driver.div(text: "Reprint Label").present?
        end

        def reprint
          button = StampsField.new(driver.span(id: "sdc-printwin-printbtn-btnInnerEl"))
          10.times do
            button.click
            button.click
