  ...
  module Orders
    module ShipFrom
      ...
      class ManageShippingAddresses < WebApps::Base
        ...
        def close_button
          cache[:close_button] = StampsField.new(driver.span(css: "span[class*=sdc-icon-sdc_idevices-close-light]")) if cache[:close_button].nil? || !cache[:close_button].present?
          cache[:close_button]
        end
        ...
        def locate_ship_from(name, company, city)
          ...
          1.upto rows do |row|
            ...
            if (grid_name.casecmp(name).zero?) && (grid_company.casecmp(company).zero?) && (grid_city.casecmp(city).zero?)
