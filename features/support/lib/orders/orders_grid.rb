    ...
    module Grid
        ...
        def scroll_to_column(name)
          ...
          case name
            ...
            when Watir::Element
              ...
              name).to be_a(String).or(eq(Symbol)).or(eq(Watir::Element))
          end
        end
        ...
        def column_number column
          5.times do
            begin
              ...
              e.backtrace.join("\n")).to eql "#{e.message}"
              e.message).to eql "Grid error. Unable to find column number for #{column}"
            end
          end
      ...
      class GridCheckBox < Column
        ...
        def check_all *args
          ...
          if args.length==1
            if args[0].is_a? Hash
              ...
              "Invalid parameter exception.  This method expects a Hash of Web Elements.").to eql ""
            end
        ...
        def check(number)
          ...
          if size > 0
            ...
            checked?(number)).to be true
          else
            "Unable to check order number #{number}").to eql "Grid is empty"
          end
        end
      ...
      class GridColumns < Browser::Modal
        ...
        def is_next_to?(left, right)
          ...
          left_column_sym).to be_truthy
          right_column_sym).to be_truthy
