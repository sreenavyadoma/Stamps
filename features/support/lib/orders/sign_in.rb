  ...
  module Orders
    ...
    class OrdersLandingPage < Browser::Modal
      ...
      def remember_my_username
        "Not yet implemented.").to eql "remember_my_username"
      end
      ...
      def first_time_sign_in usr, pw
        ...
        validation_message).to eql ""
        "Market Place modal is not present").to eql "First Time Sign In" unless market_place.present?
      end
      ...
      def orders_sign_in *args
        ...
        "Check that URL is valid: #{browser.url}").to eql "Orders Sign-in page is not loaded!" unless browser.url.include? "Orders"
        ...
        case args.length
          ...
          when 2
            ...
            "Invalid parameter. Usage: sign_in :default or sign_in username,password").to eql "Invalid Parameter: usr=#{usr}, pw=#{pw}"
        end
          ...
          begin
            if username.present?
              ...
              if invalid_username.present?
                ...
                logger.error invalid_username.text
                logger.error invalid_username.text
                "Invalid Username: #{usr}/#{pw}").to eql invalid_username.text
              end
            end
            ...
            if new_welcome.present?
              ...
              add_manual_order.present?).to be true
              import_from_csv = add_manual_order.next
              import_from_csv.present?).to be true
              import_from_stores = import_from_csv.next
              import_from_stores.present?).to be true
              learn_more = import_from_stores.next
              learn_more.present?).to be true
              learn_more.close
            end
