module Stamps
  module Orders
    class WebOrders < Browser::BaseCache
      ...
      def modals
        cache[:modals] = Stamps::Browser::StampsOrdersModals.new(param) if cache[:modals].nil?
        cache[:modals]
      end

      def marketplace
        cache[:marketplace] = Stamps::Browser::MarketPlaceStoreModals.new(param) if cache[:marketplace].nil?
        cache[:marketplace]
      end
