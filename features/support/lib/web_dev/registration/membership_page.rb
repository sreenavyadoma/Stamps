module Stamps
  module Registration
    module Membership
      ...
      #       # browser.input
      #       #
      #       # yy = StampsField.new(browser.span(text: 'some text'))
      #       # yy.scroll_into_view
      #       # browser.div
      #       # browser.span
      #       # browser.label
      ...
      #       @help_block
      #       browser.span(css: "li[id=personalinfo] div[class*='col-xs-12']:nth-child(4) div[class*=help-block] span")
      #     end
      #
      #     def single_field_help_block
      ...
      module PersonalInfo
        def first_name
          ...
          (cache[:first_name].nil?||!cache[:header].present?)?cache[:first_name]=StampsTextbox.new(browser.text_field(id: "firstName")):cache[:first_name]


        end

        def last_name
          # @last_name ||= Fields::LastName.new(browser.text_field(id: "lastName")) if @last_name.nil?||!@last_name.present?
          # @last_name
          (cache[:last_name].nil?||!cache[:header].present?)?cache[:last_name]=StampsTextbox.new(browser.text_field(id: "lastName")):cache[:last_name]
        ...
        def company
          ...
          (cache[:company].nil?||!cache[:company].present?)?cache[:company]=StampsTextbox.new(browser.text_field(id: "companyName")):cache[:company]
        end

        def address
          # @address ||= StreetAddress.new(browser.text_field(id: "street")) if @address.nil?||!@address.present?
          # @address
          (cache[:address].nil?||!cache[:address].present?)?cache[:address]= StampsTextbox.new(browser.text_field(id: "street")):cache[:address]

        end

        def city
          # @city ||= City.new(browser.text_field(id: "city")) if @city.nil?||!@city.present?
          # @city

          (cache[:city].nil?||!cache[:city].present?)?cache[:city]= StampsTextbox.new(browser.text_field(id: "city")):cache[:city]
        end
        ...
        def zip
          ...
          # @zip
          (cache[:zip].nil?||!cache[:zip].present?)?cache[:zip]= StampsTextbox.new(browser.text_field(id: "zip")):cache[:zip]

        end

        def phone
          # @phone ||= Phone.new(browser.text_field(id: "phone")) if @phone.nil?||!@phone.present?
          # @phone

          (cache[:phone].nil?||!cache[:phone].present?)?cache[:phone]= StampsTextbox.new(browser.text_field(id: "phone")):cache[:phone]
      ...
      module CreditCard
        def cc_holder_name
          # @cc_holder_name ||= CardHolderName.new(browser.text_field(id: "ccName")) if @cc_holder_name.nil?||!@cc_holder_name.present?
          # @cc_holder_name

          (cache[:cc_holder_name].nil?||!cache[:cc_holder_name].present?)?cache[:cc_holder_name]= StampsTextbox.new(browser.text_field(id: "ccName")):cache[:cc_holder_name]
        end

        def cc_number
          # @cc_number ||= CreditCardNumber.new(browser.text_field(id: "ccNumber")) if @cc_number.nil?||!@cc_number.present?
          # @cc_number
          (cache[:cc_number].nil?||!cache[:cc_number].present?)?cache[:cc_number]=StampsTextbox.new(browser.text_field(id: "ccNumber")):cache[:cc_number]
        end
        ...
        def use_mailing_for_billing
          ...
          (cache[:use_mailing_for_billing].nil?||!cache[:use_mailing_for_billing].present?)?cache[:use_mailing_for_billing]= StampsWatirCheckbox.new(browser.checkbox(name: 'useMailingAddressForBilling')):cache[:use_mailing_for_billing]
        end

        def billing_address
          # @billing_address=BillingAddress.new(browser.text_field(id: "billingStreet")) if @billing_address.nil?||!@billing_address.present?
          # @billing_address

          (cache[:billing_address].nil?||!cache[:billing_address].present?)?cache[:billing_address]= StampsTextbox.new(browser.text_field(id: "billingStreet")):cache[:billing_address]
        end

        def billing_city
          # @billing_city=BillingCity.new(browser.text_field(id: "billingCity")) if @billing_city.nil?||!@billing_city.present?
          # @billing_city
          (cache[:billing_city].nil?||!cache[:billing_city].present?)?cache[:billing_city]= StampsTextbox.new(browser.text_field(id: "billingCity")):cache[:billing_city]
        ...
        def billing_state
          ...
          (cache[:billing_state].nil?||!cache[:billing_state].present?)?cache[:billing_state]=StampsDropDownLovSubStr.new(
              browser.span(css: "button[data-id=billingState]>span"),
              browser.span(css: "button[data-id=billingState]>span"),
              browser.spans(css: "li[id=creditcard]>div>div>div[class*=billingAddressForm]>div>div[class*=menu]>ul>li>a>span[class=text]")):cache[:survey_question]

        end

        def billing_zip
          # @billing_zip=BillingZip.new(browser.text_field(id: "billingZip")) if @billing_zip.nil?||!@billing_zip.present?
          # @billing_zip
          (cache[:billing_zip].nil?||!cache[:billing_zip].present?)?cache[:billing_zip]= StampsTextbox.new(browser.text_field(id: "billingZip")):cache[:billing_zip]
        end
      end

      module TermsAndConditions
        def agree_to_terms
          # @agree_to_terms=Terms.new(browser.input(name: 'termsConditions')) if @agree_to_terms.nil?||!@agree_to_terms.present?
          # @agree_to_terms

          (cache[:agree_to_terms].nil?||!cache[:agree_to_terms].present?)?cache[:agree_to_terms]=StampsWatirCheckbox.new(browser.input(name: 'termsConditions')):cache[:agree_to_terms]
        end
        ...
        def terms_conditions_link
          ...
          (cache[:terms_conditions_link].nil?||!cache[:terms_conditions_link].present?)?cache[:terms_conditions_link]=StampsField.new(browser.a(css: "a[class*='termsLabel terms-conditions-link'")):cache[:terms_conditions_link]
        end
        ...
        def pricing_billing_details_link
          ...
          (cache[:pricing_billing_details_link].nil?||!cache[:pricing_billing_details_link].present?)?cache[:pricing_billing_details_link]=StampsField.new(browser.a(class: "pricing-billing-link")):cache[:pricing_billing_details_link]
        end
        ...
        def terms_label_1
          ...
          (cache[:terms_label_1].nil?||!cache[:terms_label_1].present?)?cache[:terms_label_1]=StampsField.new(browser.span(css: "label[for=termsConditions]>span")):cache[:terms_label_1]
        end
        ...
        def terms_label_2
          ...
          (cache[:terms_label_2].nil?||!cache[:terms_label_2].present?)?cache[:terms_label_2]=StampsField.new(browser.span(class: "termsLabel")):cache[:terms_label_2]
        end
      ...
      module Pagination
        def back_btn
          ...
          (cache[:back_btn].nil?||!cache[:back_btn].present?)?cache[:back_btn]=StampsField.new(browser.button(id: "prev")):cache[:back_btn]
