...
=end
  ...
  class ManageShippingAddresses < BatchObject
    ...
    def locate_address(name, company, city)
      ...
      1.upto rows do |row|
        ...
        if (grid_name.casecmp(name)==0) && (grid_company.casecmp(company)==0) && (grid_city.casecmp(city)==0)
          log.info "Match found! - Row #{row} :: Name=#{grid_name} :: Company=#{grid_company} :: City=#{grid_city} ::  State=#{grid_state} :: " if Stamps::Test.verbose
          return row
        else
          log.info "No match - Row #{row} :: Name=#{grid_name} :: Company=#{grid_company} :: City=#{grid_city} ::  State=#{grid_state} :: " if Stamps::Test.verbose
        end
      end
      0
    end
      ...
      begin
        ...
        if count > 1
          for row in 1..(count)
            browser_helper.click window_title, 'window_title'
            delete_row 1
            log.info "Row #{row} :: Deleting row 1..." if Stamps::Test.verbose
    ...
    def shipping_address_count
      ...
      log.info "Manage Shipping Address:: row count = #{rows.length.to_i}" if Stamps::Test.verbose
      rows.length.to_i
    end
    ...
    def checked?(row)
      ...
      log.info "Row #{row} selected? #{checked}" if Stamps::Test.verbose
      checked
    end
  ...
  class DeleteShippingAddress < BatchObject
    ...
    def delete
      5.times {
        begin
          log.info "Delete Shipping Address :: #{message_field.text}" if Stamps::Test.verbose
      ...
      5.times {
        begin
          ...
          if selection_label.present?
            ...
            log.info "#{selection_cost}" if Stamps::Test.verbose
      ...
      5.times {
        begin
          ...
          if selection_label.present?
            ...
            log.info "#{qtip}" if Stamps::Test.verbose
            return qtip
          end
        rescue
          #ignore
  ...
  class BatchService < OrderForm
    ...
    def select selection
      log.info "Select Service #{selection}" if Stamps::Test.verbose
      box = text_box
      button = drop_down
      ...
      10.times {
        begin
          ...
          log.info "Selected Service #{selected_service} - #{(selected_service.include? selection)?"done": "service not selected"}" if Stamps::Test.verbose
          break if selected_service.include? selection
        rescue
          #ignore
        end
      }
      log.info "#{selection} service selected." if Stamps::Test.verbose
      selection_label
    end
      ...
      10.times {
        begin
          ...
          if cost_label.present?
            ...
            log.info "Service Cost for \"#{selection}\" is #{service_cost}" if Stamps::Test.verbose
            return service_cost
          end
        rescue
          #ignore
      ...
      5.times {
        begin
          ...
          if selection_label.present?
            ...
            log.info "Service Tooltip for \"#{selection}\" is #{tooltip}" if Stamps::Test.verbose
            return tooltip
          end
        rescue
          #ignore
  ...
  class ShipToCountry < OrderForm
    ...
    def select country
      log.info "Select Country #{country}" if Stamps::Test.verbose
      ...
      10.times {
        begin
          ...
          log.info "Selection #{text_box.text} - #{(text_box.text.include? country)?"was selected": "not selected"}" if Stamps::Test.verbose
          break if text_box.text.include? country
        rescue
          #ignore
        end
      }
      log.info "#{country} selected." if Stamps::Test.verbose
  ...
  class OrderDetails < OrderForm
    ...
    def add_item
      ...
      log.info "Add Item Button #{(add_item.present?)?"Exist!":'DOES NOT EXIST!'}" if Stamps::Test.verbose
    ...
    def ship_cost_span
      ...
      log.info "single-order form is #{(browser_helper.present? span)?'present':'NOT present'}" if Stamps::Test.verbose
      span
    end
    ...
    def order_id
      ...
      begin
        ...
        log.info "single-order form Order ID label was not present" if Stamps::Test.verbose
      end
