module Batch
  ...
  class CustomsItemGrid < BatchObject
    ...
    def item number
      ...
      log.info "Item Count: #{line_item_count}" if Stamps::Test.verbose
      ...
      log.info "User Entered Number: #{number}. Actual Item Count: #{line_item_count}" if Stamps::Test.verbose
...
=end
  ...
  class InternalTransaction < BatchObject
    ...
    def select selection
      log.info "Select Internal Transaction Number: #{selection}" if Stamps::Test.verbose
      ...
      10.times {
        begin
          ...
          log.info "Selection #{text_box.text} - #{(text_box.text.include? selection)?"was selected": "not selected"}" if Stamps::Test.verbose
          break if text_box.text.include? selection
        rescue
          #ignore
        end
      }
      log.info "#{selection} selected." if Stamps::Test.verbose
      selection_label
    end
  end
  ...
  class CustomsForm < BatchObject
    ...
    def total_weight_error
      ...
      log.info "Total Weight dev error: #{qtip_error}" if Stamps::Test.verbose
      qtip_error
    end
    ...
    def total_weight
      ...
      log.info "Total Weight: #{weight_label.text}" if Stamps::Test.verbose
      weight_label
    end

    def total_weight_lbs
      lbs = total_weight.text.scan(/\d+/).first
      log.info "Pounds: #{lbs}" if Stamps::Test.verbose
      lbs
    end

    def total_weight_oz
      oz = total_weight.text.scan(/\d+/).last
      log.info "Ounces: #{oz}" if Stamps::Test.verbose
      oz
    end
    ...
    def verify_i_agree_checked
      ...
      log.info "I agree is #{(checked)? 'checked' : 'unchecked'}" if Stamps::Test.verbose
      checked
    end
    ...
    def i_agree_checkbox
      ...
      log.info "I Agree Checkbox is #{(browser_helper.present? text_field)?'Present' : 'Not Present'}" if Stamps::Test.verbose
      text_field
    end
    ...
    def privacy_act_statement_link
      ...
      log.info "USPS Privacy Act Statement is #{(browser_helper.present? link)?'Present' : 'Not Present'}" if Stamps::Test.verbose
      link
    end
    ...
    def restrictions_prohibitions_link
      ...
      log.info "Restrictions and Prohibitions is #{(browser_helper.present? link)?'Present' : 'Not Present'}" if Stamps::Test.verbose
      link
    end
